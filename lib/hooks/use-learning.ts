...
export function useQuestions(quizId?: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchAll<Types.Question>(
        quizId ? `learning/questions/?quiz_id=${quizId}` : 'learning/questions/'
      ),
  });
}
...
export function useSubmissions(assignmentId?: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchAll<Types.Submission>(
        assignmentId
          ? `learning/submissions/?assignment_id=${assignmentId}`
...
export function useAttempts(quizId?: string) {
  return useQuery({
    ...
    queryFn: () =>
      fetchAll<Types.Attempt>(
        quizId ? `learning/attempts/?quiz_id=${quizId}` : 'learning/attempts/'
      ),
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['submissions', (variables.data as any).assignment],
      });
    },
  });
}
