...
const normalizeTerm = (
  term: Partial<AcademicTermOption> | null | undefined
): AcademicTermOption => ({
  id: term?.id ?? '',
  name: term?.name ?? '',
...
export function usePaginatedPayments(
  ...
  const filters =
    typeof studentOrFilters === 'string' ? { studentId: studentOrFilters } : studentOrFilters || {};

  return useQuery({
    queryKey: [...queryKeys.payments, { page, pageSize, ...filters }],
    queryFn: async () => {
      const response = await fetchPaginated<PaymentApiResponse>(
        'bursary/payments/',
        page,
        pageSize,
        {
