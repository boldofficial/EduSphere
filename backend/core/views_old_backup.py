            ...
            with transaction.atomic():
                ...
                try:
                    if "current_session" in data:
                        settings_obj.current_session = data["current_session"]
                    if "current_term" in data:
                        settings_obj.current_term = data["current_term"]
                    if "school_tagline" in data:
                        settings_obj.school_tagline = data["school_tagline"]
                    if "watermark_media" in data:
                        settings_obj.watermark_media = process_base64(data["watermark_media"])

                    if "director_name" in data:
                        settings_obj.director_name = data["director_name"]
                    if "director_signature" in data:
                        settings_obj.director_signature = process_base64(data["director_signature"])
                    if "head_of_school_name" in data:
                        settings_obj.head_of_school_name = data["head_of_school_name"]
                    if "head_of_school_signature" in data:
                        settings_obj.head_of_school_signature = process_base64(data["head_of_school_signature"])

                    if "subjects_global" in data:
                        settings_obj.subjects_global = data["subjects_global"]
                    if "terms" in data:
                        settings_obj.terms_list = data["terms"]
                    if "show_position" in data:
                        settings_obj.show_position = data["show_position"]
                    if "show_skills" in data:
                        settings_obj.show_skills = data["show_skills"]
                    if "tiled_watermark" in data:
                        settings_obj.tiled_watermark = data["tiled_watermark"]
                    if "next_term_begins" in data:
                        settings_obj.next_term_begins = clean_date(data["next_term_begins"])
                    if "class_teacher_label" in data:
                        settings_obj.class_teacher_label = data["class_teacher_label"]
                    if "head_teacher_label" in data:
                        settings_obj.head_teacher_label = data["head_teacher_label"]
                    if "report_font_family" in data:
                        settings_obj.report_font_family = data["report_font_family"]
                    if "report_scale" in data:
                        settings_obj.report_scale = data["report_scale"]
                ...
                try:
                    if "landing_hero_title" in data:
                        settings_obj.landing_hero_title = data["landing_hero_title"]
                    if "landing_hero_subtitle" in data:
                        settings_obj.landing_hero_subtitle = data["landing_hero_subtitle"]
                    if "landing_features" in data:
                        settings_obj.landing_features = data["landing_features"]
                    if "landing_about_text" in data:
                        settings_obj.landing_about_text = data["landing_about_text"]
                    if "landing_hero_image" in data:
                        settings_obj.landing_hero_image = process_base64(data["landing_hero_image"])
                    if "landing_gallery_images" in data and isinstance(data["landing_gallery_images"], list):
                        settings_obj.landing_gallery_images = [
                            process_base64(img) for img in data["landing_gallery_images"]
                        ]
                ...
                try:
                    ...
                    if "promotion_threshold" in data:
                        settings_obj.promotion_threshold = to_int(data["promotion_threshold"], 50)
                    if "promotion_rules" in data:
                        settings_obj.promotion_rules = data["promotion_rules"]

                    if "show_bank_details" in data:
                        settings_obj.show_bank_details = data["show_bank_details"]
                    if "bank_name" in data:
                        settings_obj.bank_name = data["bank_name"]
                    if "bank_account_name" in data:
                        settings_obj.bank_account_name = data["bank_account_name"]
                    if "bank_account_number" in data:
                        settings_obj.bank_account_number = data["bank_account_number"]
                    if "bank_sort_code" in data:
                        settings_obj.bank_sort_code = data["bank_sort_code"]
                    if "invoice_notes" in data:
                        settings_obj.invoice_notes = data["invoice_notes"]
                    if "invoice_due_days" in data:
                        settings_obj.invoice_due_days = to_int(data["invoice_due_days"], 14)

                    if "role_permissions" in data:
                        settings_obj.role_permissions = data["role_permissions"]

                    # Also handle report_scale if present
                    if "report_scale" in data:
                        settings_obj.report_scale = to_int(data["report_scale"], 100)
...
class GlobalActivityLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Optional filtering by action or search query
