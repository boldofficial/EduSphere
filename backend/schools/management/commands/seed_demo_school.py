    ...
    def handle(self, *args, **options):
        ...
        for username, role, full_name, title in roles:
            user, u_created = User.objects.get_or_create(
                username=username, defaults={"email": f"{username}@boldideas.edu", "role": role}
            )
            if u_created:
            ...
            if role == User.Role.TEACHER:
                teacher = Teacher.objects.create(school=school, user=user, name=full_name, staff_type="ACADEMIC")
        ...
        for i in range(20):
            ...
            s_u, _ = User.objects.get_or_create(
                username=f"student_{i}", defaults={"role": User.Role.STUDENT, "school": school}
            )
            Student.objects.create(
                school=school,
        ...
        for i in range(5):  # Last 5 days
            ...
            for cls in class_objs[:2]:  # Only for some classes
                ...
                for s in students:
                    ...
                    AttendanceRecord.objects.create(school=school, attendance_session=session, student=s, status=status)
