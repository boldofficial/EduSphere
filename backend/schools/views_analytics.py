...
class SystemHealthView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {
                "status": (
                    "healthy" if redis_status == "connected" and celery_status == "active" else "partially_degraded"
                ),
...
class StrategicAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        registration_trends = (
            School.objects.filter(created_at__gte=last_year)
        ...
        revenue_trends = (
            SchoolPayment.objects.filter(status="success", date__gte=last_year)
        ...
        # 3. Plan Distribution
        plan_distribution = SubscriptionPlan.objects.annotate(school_count=Count("subscription")).values(
            "name", "school_count"
        )
...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        logs = GlobalActivityLog.objects.filter(Q(description__icontains=query) | Q(action__icontains=query))[:10]
        ...
        return Response(
            {
                ...
                "logs": [
                    {"id": l.id, "description": l.description, "action": l.action, "created_at": l.created_at}
                    for l in logs
                ],
            }
        )
