...
class PlatformModulesView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            [
                {"id": m.module_id, "name": m.name, "description": m.description, "is_active": m.is_active}
                for m in modules
            ]
        )
...
class SchoolManagementView(APIView):
    ...
    def delete(self, request, pk=None):
        ...
        try:
            ...
            GlobalActivityLog.objects.create(
                action="SCHOOL_DELETED", user=request.user, description=f"Deleted school '{school_name}' (ID: {pk})"
            )
    ...
    def put(self, request, pk=None):
        ...
        if plan_id or sub_status or sub_end_date:
            logger.info(
                f"Subscription update triggered: plan_id={plan_id}, status={sub_status}, end_date={sub_end_date}"
            )
            with transaction.atomic():
                sub, _ = Subscription.objects.get_or_create(
                    school=school, defaults={"plan": SubscriptionPlan.objects.first(), "end_date": timezone.now()}
                )
...
class SchoolRevenueView(APIView):
    ...
    def get(self, request):
        ...
        total_revenue = SchoolPayment.objects.filter(status="success").aggregate(Sum("amount"))["amount__sum"] or 0

        recent_payments = (
            SchoolPayment.objects.filter(status="success")
            .order_by("-date")[:5]
            .values("school__name", "amount", "date", "reference")
        )

        return Response({"total_revenue": total_revenue, "recent_payments": list(recent_payments)})
...
class RecordPaymentView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            # Record Payment
            payment = SchoolPayment.objects.create(school=school, amount=amount, reference=reference, status="success")
...
class AdminDemoRequestView(APIView):
    ...
    def get(self, request):
        ...
        if page is not None:
            serializer = DemoRequestSerializer(page, many=True)
            return paginator.get_paginated_response(serializer.data)
