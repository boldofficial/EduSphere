...
def approve_demo_requests(modeladmin, request, queryset):
    ...
    if email_count > 0:
        ...
        modeladmin.message_user(
            request, f"{success_count} requests approved. No emails sent (check 'demo-approved' template)."
        )
...
class DemoRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Applicant Details", {"fields": ("name", "email", "phone", "school_name", "role")}),
        ("Status", {"fields": ("status", "admin_notes", "approved_by")}),
