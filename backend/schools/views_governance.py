...
class PlatformGovernanceView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"activities": self.get_activities(), "announcements": self.get_announcements()})
    ...
    def post(self, request):
        ...
        announcement = PlatformAnnouncement.objects.create(
            title=title, message=message, priority=priority, created_by=request.user
        )

        return Response({"success": True, "id": announcement.id})
...
class UserAnnouncementsView(APIView):
    ...
    def get(self, request):
        announcements = PlatformAnnouncement.objects.filter(is_active=True, target_role=request.user.role).order_by(
            "-created_at"
        )
