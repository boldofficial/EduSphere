...
class BroadsheetPDFGenerator:
    ...
    def generate(self):
        ...
        header_style = ParagraphStyle(
            "HeaderStyle", parent=self.styles["Heading1"], alignment=TA_CENTER, fontSize=16, spaceAfter=10
        )
        elements.append(Paragraph(self.school.name.upper(), header_style))

        sub_header_style = ParagraphStyle(
            "SubHeaderStyle", parent=self.styles["Normal"], alignment=TA_CENTER, fontSize=12, spaceAfter=20
        )
        sub_text = (
            f"Master Broadsheet Report - {self.student_class.name}<br/>Session: {self.session} | Term: {self.term}"
        )
        ...
        for student in students:
            report = ReportCard.objects.filter(student=student, session=self.session, term=self.term).first()
            ...
            sheet_rows.append(
                {"name": student.names, "scores": row_scores, "total": total, "avg": avg, "pos": 0}  # Placeholder
            )
...
class ReportCardPDFGenerator:
    ...
    def generate_bulk(self, student_class, session, term):
        ...
        reports = ReportCard.objects.filter(
            school=self.school, student_class=student_class, session=session, term=term
