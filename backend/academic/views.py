...
class AITimetableGenerateView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            # 1. First Pass: Delete existing entries in bulk for the affected slots
            ...
            for entry_data in entries:
                try:
                    ...
                    # Delete selective slot (still one-by-one but we could optimize this further if needed)
                    # For now, we prioritize the Create N+1
            ...
            if admission.intake:
                try:
                    ...
                    for fee in package.fees.all():
                        StudentFee.objects.get_or_create(student=student, fee_item=fee, school=student.school)
