...
class EmailTemplate(models.Model):
    name = models.CharField(
        max_length=100, unique=True, help_text="Unique identifier for the template (e.g., 'welcome_email')"
    )
    ...
    variables = models.JSONField(
        default=dict, help_text="List of available variables for this template (e.g., {'user_name': 'John Doe'})"
    )
...
class EmailCampaign(models.Model):
    ...
    audience_filter = models.JSONField(
        default=dict, blank=True, help_text="Filters applied to select recipients (e.g. {'role': 'SCHOOL_ADMIN'})"
    )
