...
export function TakeQuizView({ quiz }: TakeQuizViewProps) {
  ...
  const [attempt, setAttempt] = useState<Types.Attempt | null>(null);
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && !isCompleted && attempt) {
        ...
        if (newCount >= 3) {
          ...
          addToast(
            `Warning: Tab switch detected! Violation ${newCount}/3. Exam will auto-submit on 3rd violation.`,
            'error',
            30000
          );
        }
  ...
  if (isLoading || !attempt)
    return <div className="p-12 text-center">Initializing Exam Session...</div>;
  ...
  if (isAlreadySubmitted) {
    return (
      <Card className="max-w-2xl mx-auto p-12 text-center space-y-6">
        <div
          className={`w-20 h-20 ${attempt.is_violated ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'} rounded-full flex items-center justify-center mx-auto`}
        >
          {attempt.is_violated ? (
        ...
        <h2 className="text-3xl font-bold">
          {attempt.is_violated ? 'Exam Terminated' : 'Exam Submitted'}
        </h2>
        ...
        <Button onClick={() => (window.location.href = '/learning/cbt')} className="w-full">
          Return to Dashboard
        </Button>
      </Card>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
        ...
        <Card className="p-8 shadow-lg border-t-4 border-t-brand-500">
          ...
          <div className="space-y-3">
            {currentQuestion.question_type === 'mcq' ? (
              currentQuestion.options?.map((opt: Types.QuestionOption) => (
                <button
                  key={opt.id}
                  onClick={() =>
                    handleOptionSelect(currentQuestion.id.toString(), opt.id.toString())
                  }
      ...
      <div className="p-4 bg-amber-50 rounded-lg border border-amber-100 flex gap-3 text-amber-800 text-sm">
        ...
        <p>
          <strong>Security Notice:</strong> This exam is monitored. Switching tabs, opening
          developer tools, or losing focus may result in automatic submission. Ensure a stable
          internet connection.
        </p>
      </div>
    </div>
