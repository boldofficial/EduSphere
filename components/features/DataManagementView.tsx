    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.settings && data.students) {
          addToast(
            'Data import is not supported with database mode. Please use Supabase dashboard for bulk imports.',
            'info'
          );
        } else {
      ...
      <Card title="System Info">
        <div className="text-sm text-gray-500 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <p>
              Storage Engine: <span className="font-medium text-green-600">Supabase Database</span>
            </p>
          </div>
          <p>Last Sync: {new Date().toLocaleString()}</p>
          <p className="text-xs mt-4">All data is stored securely in the cloud database.</p>
        </div>
      </Card>
    </div>
  );
