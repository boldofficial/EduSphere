...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const initializeForm = (s: Types.Settings) => ({
    ...
    head_teacher_label:
      (s.head_teacher_label === 'Head Teacher' ? 'Head of Schools' : s.head_teacher_label) ??
      'Head of Schools',
  ...
  const initializePaymentSettings = (
    s?: Types.SchoolPaymentSettings | null
  ...
  const [paymentForm, setPaymentForm] = useState<Types.SchoolPaymentSettings>(
    initializePaymentSettings(paymentSettings)
  );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">
            Configure school identity, session details, and report card assets.
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="show_position" className="text-sm font-medium text-gray-700">
                  Display Student Rank/Position
                </label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="h-4 w-4 text-brand-600 border-gray-300 rounded"
                />
                <label htmlFor="show_skills" className="text-sm font-medium text-gray-700">
                  Display Skills & Behavior Domain
                </label>
              </div>
              <div className="pt-2">
                <Select
              ...
              <div className="pt-2 space-y-2">
                <label className="text-sm font-medium text-gray-700 block">
                  Report Card Scale: {formData.report_scale}%
                </label>
                <input
                  type="range"
                  min="70"
              ...
              <div className="flex items-center gap-2 text-brand-600 border-t pt-3">
                <input
                  ...
                  className="h-4 w-4 text-brand-600 border-gray-300 rounded"
                />
                <label htmlFor="tiled_watermark" className="text-sm font-bold">
                  Tile Watermark (Repeating)
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 border-t pt-4">
              <div>
                ...
                {formData.logo_media && (
                  <img
                    src={Utils.getMediaUrl(formData.logo_media) || ''}
              ...
              <div>
                ...
                />
                {formData.watermark_media && (
                  <img
                    src={Utils.getMediaUrl(formData.watermark_media) || ''}
                    className="mt-2 h-12 object-contain border"
                  />
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t pt-4">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">
                  Director Signature (for ID Card)
                </label>
                <Input
                ...
                <input
                  ...
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, 'director_signature')}
                />
                {formData.director_signature && (
                  <img
                    src={Utils.getMediaUrl(formData.director_signature) || ''}
                    className="mt-2 h-10 object-contain border border-dashed p-1"
                  />
                )}
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">
                  {formData.head_teacher_label} Signature
                </label>
                <Input
                  placeholder="Head of School Name"
                  value={formData.head_of_school_name || ''}
                  onChange={(e) => handleChange('head_of_school_name', e.target.value)}
                  className="mt-1"
                />
                <input
                  type="file"
                ...
                />
                {formData.head_of_school_signature && (
                  <img
                    src={Utils.getMediaUrl(formData.head_of_school_signature) || ''}
                    className="mt-2 h-10 object-contain border border-dashed p-1"
                  />
                )}
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-4 w-4 text-brand-600 border-gray-300 rounded"
              />
              <label htmlFor="show_bank_details" className="text-sm font-medium text-gray-700">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Input
                label="Due Days"
                type="number"
                value={formData.invoice_due_days?.toString() || '14'}
                onChange={(e) => handleChange('invoice_due_days', parseInt(e.target.value) || 14)}
              />
            </div>
            <textarea
