...
export const StaffDashboardView = () => {
  ...
  const currentTermExpenses = useMemo(
    () => expenses.filter((e) => e.session === targetSession && e.term === targetTerm).slice(0, 5),
    [expenses, targetSession, targetTerm]
  );

  // Recent payments from current term
  const currentTermPayments = useMemo(
    () => payments.filter((p) => p.session === targetSession && p.term === targetTerm).slice(0, 5),
    [payments, targetSession, targetTerm]
  );

  // Recent announcements
  const recentAnnouncements = useMemo(
    () => announcements.filter((a) => a.target === 'all' || a.target === 'staff').slice(0, 3),
    [announcements]
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            {myProfile ? `Welcome, ${myProfile.name}` : 'Non-Teaching Staff Portal'} | {targetTerm},{' '}
            {targetSession}
          </p>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-gray-900">
              {Utils.formatCurrency(totalPaymentsThisTerm)}
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-gray-900">
              {Utils.formatCurrency(totalExpensesThisTerm)}
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
              {currentTermExpenses.length > 0 ? (
                currentTermExpenses.map((expense) => (
                  <div
                    key={expense.id}
                ...
                {myProfile?.assigned_modules && myProfile.assigned_modules.length > 0 ? (
                  myProfile.assigned_modules.map((module: string) => (
                    <a
                      key={module}
                      href={`/${module}`}
                      className="block p-3 bg-white/10 rounded-xl hover:bg-white/20 transition-colors uppercase text-xs font-bold tracking-wider"
                    >
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">
                    {myProfile.tasks || 'General duties'}
                  </p>
                </div>
                <div>
