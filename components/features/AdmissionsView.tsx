...
export const AdmissionsView: React.FC<AdmissionsViewProps> = ({
  ...
  const updateStatus = (id: string | number, status: Types.Admission['status']) => {
    ...
    onUpdate(updated);
    addToast(
      `Application ${status === 'accepted' ? 'accepted' : status === 'rejected' ? 'rejected' : 'updated'}`,
  ...
  const getProgramBadge = (program: Types.Admission['program']) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-bold ${styles[program]}`}>
        {labels[program]}
      </span>
    );
  };
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-gray-100 p-1 rounded-lg">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-md flex items-center gap-1.5 transition-colors ${viewMode === 'kanban' ? 'bg-white shadow text-brand-700' : 'text-gray-500'}`}
            >
              <LayoutGrid className="h-3.5 w-3.5" /> Pipeline
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`px-3 py-1.5 text-xs font-medium rounded-md flex items-center gap-1.5 transition-colors ${viewMode === 'list' ? 'bg-white shadow text-brand-700' : 'text-gray-500'}`}
            >
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-4 py-2 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-brand-500"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {(['pending', 'reviewed', 'accepted', 'rejected'] as const).map((status) => {
            ...
            return (
              <div
                key={status}
                className={`${colors.bg} rounded-xl border ${colors.border} p-3 min-h-[300px]`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className={`h-2.5 w-2.5 rounded-full ${colors.dot}`} />
                    <h3 className={`font-bold text-sm uppercase tracking-wide ${colors.header}`}>
                      {status}
                    </h3>
                  </div>
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded-full ${colors.bg} ${colors.header}`}
                  >
                    {columnItems.length}
                  </span>
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Applied for {admission.class_applied} • {getProgramBadge(admission.program)}
                      </p>
                      <p className="text-xs text-gray-400 mt-1">
                        Submitted {new Date(admission.created_at).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <User size={18} /> Child Information
                </h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-500">Name:</span>{' '}
                    <span className="font-medium">{selectedAdmission.child_name}</span>
                  </div>
                  <div>
                    <span className="text-gray-500">Gender:</span>{' '}
                    <span className="font-medium">{selectedAdmission.child_gender}</span>
                  </div>
                  <div>
                    <span className="text-gray-500">Date of Birth:</span>{' '}
                    <span className="font-medium">{selectedAdmission.child_dob}</span>
                  </div>
                  <div>
                    <span className="text-gray-500">Previous School:</span>{' '}
                    <span className="font-medium">
                      {selectedAdmission.previous_school || 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <GraduationCap size={18} /> Program
                </h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-500">Program:</span>{' '}
                    {getProgramBadge(selectedAdmission.program)}
                  </div>
                  <div>
                    <span className="text-gray-500">Class:</span>{' '}
                    <span className="font-medium">{selectedAdmission.class_applied}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <User size={18} /> Parent/Guardian
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    <User size={14} className="text-gray-400" /> {selectedAdmission.parent_name} (
                    {selectedAdmission.relationship})
                  </div>
                  <div className="flex items-center gap-2">
                    <Mail size={14} className="text-gray-400" /> {selectedAdmission.parent_email}
                  </div>
                  <div className="flex items-center gap-2">
                    <Phone size={14} className="text-gray-400" /> {selectedAdmission.parent_phone}
                  </div>
                  <div className="flex items-center gap-2 md:col-span-2">
                    <MapPin size={14} className="text-gray-400" />{' '}
                    {selectedAdmission.parent_address}
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-gray-400 flex items-center gap-4">
                ...
                {selectedAdmission.reviewed_at && (
                  <span className="flex items-center gap-1">
                    ...
                    {new Date(selectedAdmission.reviewed_at).toLocaleString()}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex flex-wrap gap-3 justify-end rounded-b-2xl">
              <Button variant="secondary" onClick={() => setSelectedAdmission(null)}>
                Close
              </Button>
              ...
              {selectedAdmission.status === 'accepted' ? (
                ...
                <>
                  {selectedAdmission.status === 'pending' && (
                    <Button
                      variant="secondary"
                      onClick={() => updateStatus(selectedAdmission.id, 'reviewed')}
                    >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Convert <strong>{selectedAdmission.child_name}</strong> to an active student. This
              will automatically assign bundled fees for the current intake.
            </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConversionData({ ...conversionData, student_no: e.target.value })
                  }
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setConversionData({ ...conversionData, class_id: e.target.value })
                  }
                >
            ...
            <div className="mt-8 flex gap-3">
              <Button variant="secondary" className="flex-1" onClick={() => setIsConverting(false)}>
                Cancel
              </Button>
              <Button
