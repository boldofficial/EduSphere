...
export const ReportPreviewTab: React.FC<ReportPreviewTabProps> = ({
  ...
  const handleCopyParentSummary = async () => {
    ...
    const isEarlyYears = currentClass
      ? currentClass.report_mode === 'early_years' || currentClass.category === 'Nursery'
      : false;
    ...
    const secureCount = (previewScore.early_years_observations || []).filter(
      (item) => item.status === 'Secure'
    ).length;
    const developingCount = (previewScore.early_years_observations || []).filter(
      (item) => item.status === 'Developing'
    ).length;
    const emergingCount = (previewScore.early_years_observations || []).filter(
      (item) => item.status === 'Emerging'
    ).length;
        ...
        <div className="overflow-hidden rounded-3xl border border-brand-100 bg-white shadow-sm">
          <div className="bg-gradient-to-r from-brand-900 via-brand-700 to-brand-500 px-5 py-4 text-white">
            ...
            <p className="mt-1 text-xs text-white/75">
              Preview one student or generate all reports.
            </p>
          </div>
        ...
        {reportStudentId === 'all' && currentClass ? (
          ...
          <div className="flex min-h-[560px] flex-col items-center justify-center rounded-3xl border border-dashed border-brand-200 bg-gradient-to-br from-brand-50/60 via-white to-accent-50/40 px-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900">
              Select a student to preview a report card
            </h3>
            <p className="mt-2 max-w-md text-sm text-gray-500">
              Use the left panel to choose a learner, or switch to &apos;All Students&apos; for bulk
