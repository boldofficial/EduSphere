...
export const BulkScoreImport: React.FC<BulkScoreImportProps> = ({
  ...
  const getCsvTemplate = () => {
    ...
    const exampleRows = students
      .slice(0, 3)
      .map((s) => `${s.student_no},${selectedSubject || 'Mathematics'},0,0,0`)
      .join('\n');
    return `${headers}\n${exampleRows}`;
  };
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (ca1 > 20 || ca2 > 20 || exam > 60) {
        parseErrors.push({
          row: i + 1,
          studentNo,
          error: 'Score exceeds maximum (CA:20, Exam:60)',
        });
        continue;
      }
        ...
        {parsedData.length > 0 && (
          <div className="space-y-4">
            ...
            {errors.length > 0 && (
              <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                ...
                <div className="max-h-40 overflow-y-auto space-y-1">
                  ...
                  {errors.length > 10 && (
                    <p className="text-xs text-gray-500">...and {errors.length - 10} more errors</p>
                  )}
                </div>
              </div>
            )}
            ...
            {parsedData.length > 20 && (
              <p className="text-sm text-gray-500">...and {parsedData.length - 20} more rows</p>
            )}
