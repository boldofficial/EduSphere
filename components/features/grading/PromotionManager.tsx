...
export const PromotionManager: React.FC = () => {
  ...
  const activeStudents = students.filter((s: Types.Student) =>
  ...
  const studentStats = useMemo(() => {
    return activeStudents
      .map((student: Types.Student) => {
        ...
        const allAverages = termScores
          .map((s: Types.Score) => s.average)
          .filter((a: number) => a > 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage end-of-session student promotions</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <Card className="lg:col-span-1">
          <div className="p-4 space-y-4">
            ...
            {processedCount > 0 && (
              <Button className="w-full" variant="outline" onClick={handleSavePromotions}>
        ...
        <Card className="lg:col-span-3">
          <div className="p-4">
            <h3 className="font-semibold text-gray-700 mb-4">Students in {currentClass?.name}</h3>
                  ...
                  return (
                    <tr key={stat.student.id} className="border-t">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`font-bold ${
                            stat.eligible ? 'text-green-600' : 'text-red-600'
                          }`}
                        >
                        ...
                        {stat.promotedTo ? (
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                              isPromoted ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                            }`}
                          >
                            {isPromoted ? (
                              <>
