...
export const TermComparisonView: React.FC<TermComparisonViewProps> = ({
  ...
  const terms = useMemo(
    () => settings.terms || ['First Term', 'Second Term', 'Third Term'],
    [settings.terms]
  );
  ...
  const classTermData = useMemo(() => {
    ...
    return terms.map((term) => {
      ...
      const classAverage =
        termScores.length > 0 ? termScores.reduce((a, b) => a + b, 0) / termScores.length : 0;
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md ${
              compareMode === 'student' ? 'bg-white shadow text-brand-700' : 'text-gray-600'
            }`}
          >
            By Student
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md ${
              compareMode === 'class' ? 'bg-white shadow text-brand-700' : 'text-gray-600'
            }`}
          >
            By Class
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {compareMode === 'student' && selectedStudent && subjectComparison.length > 0 && (
            <Card>
              <div className="p-4">
                <h3 className="font-semibold text-gray-700 mb-4">Subject-wise Comparison</h3>
          ...
          <Card>
            <div className="p-4">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(compareMode === 'student' ? studentTermData : classTermData).map(
                    (row, index, arr) => {
                      ...
                      return (
                        <tr key={row.term} className="border-t">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`font-bold ${
                                row.average >= 50 ? 'text-green-600' : 'text-red-600'
                              }`}
                            >
