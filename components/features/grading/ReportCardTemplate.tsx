...
const getBand = (value: number) =>
  GRADE_BANDS.find((band) => value >= band.min && value <= band.max) ||
...
export const ReportCardTemplate: React.FC<ReportCardTemplateProps> = ({
  ...
  const themePrimary = isHexColor(settings?.landing_primary_color)
    ? settings.landing_primary_color
    : '#103c68';
  ...
  const subjectRows = (
    subjects.length ? subjects : score?.rows?.map((row) => row.subject) || []
  ).map((subject) => {
  ...
  const earlySecurePercent = earlyObservations.length
    ? (secureCount / earlyObservations.length) * 100
    : 0;
  ...
  const resumptionDate = settings?.next_term_begins
    ? new Date(settings.next_term_begins).toLocaleDateString()
    : 'To Be Announced';
  ...
  const progressionRows = historyScores
    ...
    .map((item, index, arr) => {
      ...
      const currentSecureCount = currentObservations.filter(
        (obs) => obs.status === 'Secure'
      ).length;
      const currentSecurePercent = currentObservations.length
        ? (currentSecureCount / currentObservations.length) * 100
        : 0;
      const previousObservations = previous?.early_years_observations || [];
      const previousSecureCount = previousObservations.filter(
        (obs) => obs.status === 'Secure'
      ).length;
      const previousSecurePercent = previousObservations.length
        ? (previousSecureCount / previousObservations.length) * 100
        : 0;

      const previousAverage = previous
        ? isEarlyYears
          ? previousSecurePercent
          : Number(previous.average || 0)
        : 0;
  ...
  const trendHistory = progressionRows.slice(-3).map((item) => ({
  ...
  const interventionAreas = isEarlyYears
    ...
    : [...sortedSubjects]
        .reverse()
        .filter((item) => item.total < 60)
        .slice(0, 2);

  const actionPlan: string[] = [];
  if (attendancePercent < 75)
    actionPlan.push('Target at least 85% attendance next term with weekly tracking.');
  if (interventionAreas.length > 0)
    actionPlan.push(
      `Schedule focused support for ${interventionAreas.map((item) => item.subject).join(' and ')}.`
    );
  if (isEarlyYears) {
    if (secureCount < Math.ceil(Math.max(1, earlyObservations.length / 2))) {
      actionPlan.push(
        'Increase home-school reading and fine-motor routines with weekly feedback notes.'
      );
    }
  } else if (average < 65) {
  ...
  if (actionPlan.length === 0)
    actionPlan.push('Maintain momentum with enrichment tasks and peer mentoring opportunities.');

  const standoutBadges = (
    isEarlyYears
      ? [
          secureCount >= Math.max(3, Math.ceil(earlyObservations.length * 0.6))
            ? 'Strong Development Milestones'
            : null,
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded border border-slate-300 bg-white">
              {settings?.logo_media ? (
                <img
                  src={Utils.getMediaUrl(settings.logo_media) || ''}
                  alt="School logo"
            ...
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-slate-500">
                {settings?.school_address || 'Address not provided'}
              </p>
            </div>
          </div>

          <div className="rounded-lg border border-slate-200 bg-[#f8fbff] px-3 py-2 text-right text-[10px]">
            <p className="font-extrabold uppercase tracking-[0.14em] text-slate-500">
      ...
      <div
        className="px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.12em] text-white"
        style={{ backgroundColor: themeDeep }}
      >
        Student Report Card
      </div>
      ...
      <div className="border-b border-slate-200 px-4 py-3">
          ...
          <div className="grid grid-cols-12 gap-3">
            ...
            <div className="col-span-7 grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
              ...
              <div>
                ...
                <span className="ml-1 font-semibold text-slate-800">
                  {score?.position ? Utils.ordinalSuffix(score.position) : '-'}
                </span>
              </div>
            </div>
        ...
        >
          <div
            ...
            style={{ backgroundColor: toRgba(themePrimary, 0.14) }}
          />
          <p
            className="relative z-10 text-[9px] font-bold uppercase tracking-[0.14em]"
            style={{ color: themeDeep }}
          >
            Term Summary
          </p>
          <p className="relative z-10 mt-1 font-medium">
            {score?.ai_performance_remark ||
              score?.teacher_remark ||
              'A commendable termly performance. Continue to build discipline, confidence, and consistency in all learning tasks.'}
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto border-b border-slate-200">
          <table className="w-full min-w-[760px] border-collapse text-[10px]">
            <thead>
              <tr>
                <th
                  className="border border-slate-200 px-2 py-2 text-left font-bold uppercase tracking-wide text-white"
                  style={{ backgroundColor: themePrimary }}
                >
                  Learning Area
                </th>
                <th className="border border-slate-200 bg-[#dcfce7] px-2 py-2 text-center font-bold uppercase tracking-wide text-emerald-700">
                  Status
                </th>
                <th className="border border-slate-200 bg-[#eef4ff] px-2 py-2 text-left font-bold uppercase tracking-wide text-[#0f355d]">
                  Observation
                </th>
                <th className="border border-slate-200 bg-[#fff7ed] px-2 py-2 text-left font-bold uppercase tracking-wide text-amber-700">
                  Next Step
                </th>
              </tr>
            </thead>
              ...
              {(earlyObservations.length
                ...
                <tr key={row.area} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fbfdff]'}>
                  ...
                  <td className="border border-slate-200 px-2 py-2 text-center">
                    <span
                      className={`inline-flex min-w-16 items-center justify-center rounded-full border px-2 py-0.5 text-[10px] font-bold ${
                        row.status === 'Secure'
          ...
          <table className="w-full min-w-[760px] border-collapse text-[10px]">
            <thead>
              <tr>
                ...
                <th className="border border-slate-200 bg-[#d7e9ff] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#0f355d]">
                  Homework
                </th>
                <th className="border border-slate-200 bg-[#d9f0ff] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#0f355d]">
                  CA
                </th>
                <th className="border border-slate-200 bg-[#ffe5c7] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#6a3b00]">
                  Exam
                </th>
                <th className="border border-slate-200 bg-[#f1f6ff] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#0f355d]">
                  Total
                </th>
                <th className="border border-slate-200 bg-[#e1ffe9] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#0f5a30]">
                  Grade
                </th>
                <th className="border border-slate-200 bg-[#f5f8fb] px-2 py-2 text-center font-bold uppercase tracking-wide text-[#4a6078]">
                  Remark
                </th>
              </tr>
            </thead>
            <tbody>
              {subjectRows.map((row, index) => (
                <tr key={row.subject} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fbfdff]'}>
                  ...
                  <td className="border border-slate-200 px-2 py-2 text-center">
                    <span
                      className={`inline-flex min-w-9 items-center justify-center rounded-full border px-2 py-0.5 text-[10px] font-bold ${row.band.badgeClass}`}
                    >
                      {row.band.grade}
                    </span>
                  </td>
        ...
        {isEarlyYears ? (
          <>
            ...
            <div className="rounded border border-[#ecd7ff] bg-[#f7efff] px-2 py-2">
              <p className="text-[9px] font-semibold uppercase tracking-wide text-[#8054a6]">
                Attendance
              </p>
          ...
          <>
            <div className="rounded border border-[#d8e5f5] bg-[#e9f2ff] px-2 py-2">
              <p className="text-[9px] font-semibold uppercase tracking-wide text-[#46658a]">
                Total Score
              </p>
              <p className="mt-1 text-[14px] font-bold" style={{ color: themeDeep }}>
                {totalAggregate}
              </p>
            </div>
            ...
            <div className="rounded border border-[#dbe8ff] bg-[#eef4ff] px-2 py-2">
              <p className="text-[9px] font-semibold uppercase tracking-wide text-[#4d67a1]">
                Position
              </p>
              <p className="mt-1 text-[14px] font-bold text-[#3558a2]">
                {score?.position ? Utils.ordinalSuffix(score.position) : '-'}
              </p>
            </div>
            <div className="rounded border border-[#ecd7ff] bg-[#f7efff] px-2 py-2">
      ...
      <div className="border-b border-slate-200 px-4 py-3">
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          ...
          >
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-[10px]">
              <span
                ...
                style={{ backgroundColor: '#eef4ff', color: themeDeep }}
              >
                {trendText}
              </span>
              {trendValues.length > 1 && (
                <span
                  className={`font-semibold ${trendDelta >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                >
                  {trendDelta >= 0 ? '+' : ''}
                  {trendDelta.toFixed(1)} points across recent terms
                </span>
              )}
            </div>
          ...
          <div className="rounded border border-slate-200 p-3">
            ...
            <p className="text-[10px] leading-relaxed text-slate-600">
              {score?.ai_performance_remark ||
                'Insights generated from academic performance, attendance, and class behavior for targeted improvement.'}
            </p>
              ...
              <div className="rounded border border-amber-100 bg-amber-50 p-2">
                ...
                <ul className="mt-1 space-y-0.5 text-slate-600">
                  {interventionAreas.length > 0 ? (
                    ...
                    <li>- Maintain current progress</li>
                  )}
                </ul>
              </div>
            </div>
        ...
        <div className="mt-3 overflow-hidden rounded border border-slate-200">
          <div
            className="flex items-center justify-between px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-white"
            style={{ backgroundColor: themePrimary }}
          >
          ...
          {progressionRows.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[620px] border-collapse text-[10px]">
                <thead>
                  <tr>
                    <th className="border border-slate-200 bg-[#f3f7ff] px-2 py-2 text-left font-bold text-slate-600">
                      Session
                    </th>
                    <th className="border border-slate-200 bg-[#f3f7ff] px-2 py-2 text-left font-bold text-slate-600">
                      Term
                    </th>
                    <th className="border border-slate-200 bg-[#eaffef] px-2 py-2 text-center font-bold text-emerald-700">
                      {isEarlyYears ? 'Secure %' : 'Average'}
                    </th>
                    <th className="border border-slate-200 bg-[#eef4ff] px-2 py-2 text-center font-bold text-[#3f5f98]">
                      Position
                    </th>
                    <th className="border border-slate-200 bg-[#f7efff] px-2 py-2 text-center font-bold text-[#7a4aa3]">
                      Attendance
                    </th>
                    <th className="border border-slate-200 bg-[#fff3df] px-2 py-2 text-center font-bold text-amber-700">
                      Growth
                    </th>
                    <th className="border border-slate-200 bg-[#f8fbff] px-2 py-2 text-center font-bold text-slate-600">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {progressionRows.map((row, index) => (
                    <tr
                      key={row.id || `${row.session}-${row.term}`}
                    ...
                    >
                      ...
                      <td className="border border-slate-200 px-2 py-1.5 text-center font-bold text-emerald-700">
                        {row.average.toFixed(1)}%
                      </td>
                      <td className="border border-slate-200 px-2 py-1.5 text-center font-semibold text-[#385ea4]">
                        {row.position ? Utils.ordinalSuffix(row.position) : '-'}
                      </td>
                      <td className="border border-slate-200 px-2 py-1.5 text-center font-semibold text-[#6d3e9c]">
                        {row.attendance === null ? '-' : `${row.attendance}%`}
                      </td>
                      <td className="border border-slate-200 px-2 py-1.5 text-center">
                        <span
                          className={`font-semibold ${row.growth === null ? 'text-slate-500' : row.growth >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}
                        >
                          {row.growth === null
                            ? '-'
                      ...
                      <td className="border border-slate-200 px-2 py-1.5 text-center">
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-0.5 font-semibold ${row.isPassed ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}
                        >
            ...
            <p className="px-3 py-3 text-[10px] text-slate-500">
              No historical term data yet. Progression table will populate once more results are
              recorded.
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] text-slate-600">
              ...
              <span className="font-bold" style={{ color: themeDeep }}>
                {verificationCode}
              </span>
              ...
              <span className="font-mono tracking-[0.06em] text-slate-700">
                {resultFingerprint}
              </span>
            </div>
            <p className="mt-2 text-[9px] text-slate-500">
              Scan the QR code or quote the Verification ID at the school office to confirm this
              report card.
            </p>
          </div>
      ...
      <div className="border-b border-slate-200 px-4 py-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="overflow-hidden rounded border border-slate-200">
            <div
              className="px-2 py-1.5 text-[10px] font-bold uppercase tracking-wide text-white"
              style={{ backgroundColor: themePrimary }}
            >
              Affective Domain
            </div>
          ...
          <div className="mt-3 rounded border border-slate-200 bg-[#f9fbfd] px-3 py-2 text-[10px]">
            ...
            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-slate-600">
              {GRADE_BANDS.map((band) => (
                <span key={band.grade}>
                  <span className="font-bold text-slate-700">{band.grade}</span> ({band.min}-
                  {band.max})
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 gap-3 px-4 py-3">
        <div className="rounded border border-slate-200 p-3">
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">
            {settings?.class_teacher_label || 'Class Teacher'} Remark
          </p>
          <p className="mt-2 min-h-14 text-[10px] leading-relaxed text-slate-600">
            {score?.teacher_remark ||
              'Steady progress this term. Continue to improve class participation and independent study habits.'}
          </p>
          <div className="mt-4 border-t border-dashed border-slate-300 pt-2 text-right text-[9px] text-slate-400">
            Signature
          </div>
        </div>
        ...
        <div className="rounded border border-slate-200 p-3">
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">
            {settings?.head_teacher_label || 'Head Teacher'} Remark
          </p>
          <p className="mt-2 min-h-14 text-[10px] leading-relaxed text-slate-600">
            {score?.head_teacher_remark ||
              'A promising performance. Keep striving for excellence with consistency and discipline.'}
          </p>
          <div className="mt-4 flex items-center justify-end border-t border-dashed border-slate-300 pt-2">
            {settings?.head_of_school_signature ? (
              <img
                src={Utils.getMediaUrl(settings.head_of_school_signature) || ''}
                alt="Head signature"
