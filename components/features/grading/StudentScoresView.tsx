      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500">
            View your scores for {targetTerm} - {targetSession}
          </p>
        </div>
        ...
        <Card className="p-12 text-center">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-500 mb-6">
              Your results for {targetTerm} ({targetSession}) have not been released yet. Please
              check back later or contact your school administration.
            </p>
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="text-3xl font-bold">
                  {isEarlyYears
                    ? earlySecurePercent.toFixed(1)
                    : myScore?.average?.toFixed(1) || '0'}
                  %
                </p>
                <TrendBadge trend={myScore?.performance_trend} showText={false} />
              </div>
              <p className="text-xs text-white/80">
                {isEarlyYears ? 'Secure Development' : 'Term Average'}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {position ? Utils.ordinalSuffix(position) : '-'}
              </p>
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {isEarlyYears
                  ...
                  : myScore?.rows?.length || 0}
              </p>
              <p className="text-xs text-gray-500">
                {isEarlyYears ? 'Learning Areas' : 'Subjects'}
              </p>
            </div>
          </div>
        </Card>
            ...
            <table className="w-full">
              ...
              <tbody>
                {myScore.rows.map((row, i) => (
                  <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-bold ${getGradeColor(row.grade)}`}
                      >
                        {row.grade}
                      </span>
                    </td>
              ...
              <tfoot>
                <tr className="bg-gray-50 font-bold">
                  ...
                  <td className="py-3 px-4 text-center text-brand-600 text-lg">
                    {myScore.average?.toFixed(1)}%
                  </td>
                  <td className="py-3 px-4 text-center">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-bold ${getGradeColor(myScore.rows[0]?.grade || 'C')}`}
                    >
                      {myScore.average && myScore.average >= 70
                        ? 'A'
                        : myScore.average && myScore.average >= 60
                          ? 'B'
                          : myScore.average && myScore.average >= 50
                            ? 'C'
                            : myScore.average && myScore.average >= 45
                              ? 'D'
                              : 'F'}
                    </span>
                  </td>
