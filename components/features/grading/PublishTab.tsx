          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-amber-800">
                {classPublishStatus.totalCount - classPublishStatus.passedCount}
              </p>
            </div>
          </div>
        </Card>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-amber-700">
                  Students and parents can only view report cards after publication. Confirm scores
            ...
            <table className="w-full min-w-[760px] text-sm">
              <thead>
                <tr className="bg-brand-900 text-left text-[11px] font-semibold uppercase tracking-[0.14em] text-white">
                  ...
                  <th className="px-4 py-3 text-center">
                    {isEarlyYears ? 'Learning Areas' : 'Subjects Graded'}
                  </th>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {activeStudents.map((student) => {
                  ...
                  const observationsCompleted =
                    studentScore?.early_years_observations?.filter((item) =>
                      Boolean(item.comment?.trim())
                    ).length || 0;
                  const secureCount =
                    studentScore?.early_years_observations?.filter(
                      (item) => item.status === 'Secure'
                    ).length || 0;
                  const securePercent =
                    (studentScore?.early_years_observations?.length || 0) > 0
                  ...
                  const progressTotal = isEarlyYears
                    ? Utils.EARLY_YEARS_LEARNING_AREAS.length
