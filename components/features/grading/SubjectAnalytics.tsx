...
export const SubjectAnalytics: React.FC<SubjectAnalyticsProps> = ({
  ...
  const currentClass = useMemo(
    () => classes.find((c) => Utils.sameId(c.id, classId)),
    [classes, classId]
  );
  ...
  const subjectStats = useMemo(() => {
    return classSubjects.map((subject) => {
      const subjectScoresArr = scores
        .filter(
          (s) => Utils.sameId(s.class_id, classId) && s.session === session && s.term === term
        )
      ...
      const gradeDist =
        subjectScoresArr.length > 0 && gradingScheme
          ? Stats.getGradeDistribution(subjectScoresArr, gradingScheme)
          : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-gradient-to-br from-blue-50 to-white">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {students.filter((s) => Utils.sameId(s.class_id, classId)).length}
          </p>
        </Card>
              ...
              {subjectStats.map((s) => (
                <tr key={s.subject} className="border-t">
                  ...
                  <td className="px-3 py-2 text-center">
                    <span
                      className={`font-bold ${s.average >= 70 ? 'text-green-600' : s.average >= 50 ? 'text-blue-600' : 'text-red-600'}`}
                    >
                      {s.average.toFixed(1)}%
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-2 text-center">
                    <span
                      className={`px-2 py-0.5 rounded text-xs ${s.difficulty > 10 ? 'bg-red-100 text-red-700' : s.difficulty > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}
                    >
                    ...
                    {s.gradeDistribution
                      ...
                      .map((g) => (
                        <div key={g.grade} className="flex items-center gap-2 text-xs">
                          <span className="w-4 font-bold" style={{ color: g.color }}>
                            {g.grade}
                          </span>
