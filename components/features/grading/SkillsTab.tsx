...
interface SkillsTabProps {
  ...
  onMagicRemark: (
    studentId: string,
    targetField: 'teacher_remark' | 'head_teacher_remark'
  ) => Promise<string | null>;
}
...
export const SkillsTab: React.FC<SkillsTabProps> = ({
  ...
  const commitAttendanceValue = (
    field: 'attendance_present' | 'attendance_total',
    rawValue: string
  ) => {
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
        ...
        <div className="overflow-hidden rounded-3xl border border-brand-100 bg-white shadow-sm">
          <div className="bg-gradient-to-r from-brand-900 via-brand-700 to-brand-500 px-5 py-4 text-white">
            ...
            <p className="mt-1 text-xs text-white/75">
              Capture behavior, psychomotor ratings and remarks.
            </p>
          </div>
      ...
      <div className="space-y-6 lg:col-span-3">
        {reportStudentId ? (
          <>
            <Card
              className="rounded-3xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {Utils.DOMAINS_AFFECTIVE.map((trait) => {
                  ...
                  return (
                    <div
                      key={trait}
                      className="flex items-center justify-between rounded-xl border border-amber-100 bg-white px-3 py-2.5"
                    >
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((num) => (
                          <button
                            key={num}
                            onClick={() =>
                              handleTraitChange(reportStudentId, 'affective', trait, num)
                            }
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((num) => (
                          <button
                            key={num}
                            onClick={() =>
                              handleTraitChange(reportStudentId, 'psychomotor', skill, num)
                            }
                            className={`h-8 w-8 rounded-md border text-sm font-bold transition-colors ${
                              value === num
                                ? 'border-brand-600 bg-brand-600 text-white'
          ...
          <div className="flex min-h-[520px] flex-col items-center justify-center rounded-3xl border border-dashed border-brand-200 bg-gradient-to-br from-brand-50/60 via-white to-accent-50/30 px-6 text-center">
            ...
            <p className="mt-2 max-w-md text-sm text-gray-500">
              Pick a learner from the left panel to set trait ratings, attendance, and official
              remarks.
            </p>
          </div>
        )}
