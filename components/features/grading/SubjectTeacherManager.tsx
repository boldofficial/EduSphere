          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {classes.map((cls) => {
              const assignments = subjectTeachers.filter(
                (st) =>
                  String(st.class_id) === String(cls.id) &&
                  String(st.session) === String(settings.current_session)
              );
              const subjects = Utils.getSubjectsForClass(cls);
              const coverage =
                subjects.length > 0 ? Math.round((assignments.length / subjects.length) * 100) : 0;

              return (
                <div
                  key={cls.id}
                  className={`p-3 rounded-lg border cursor-pointer transition-colors ${
                    Utils.sameId(selectedClass, cls.id)
