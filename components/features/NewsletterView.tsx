...
export const NewsletterView: React.FC = () => {
  ...
  const togglePublish = (newsletter: Types.Newsletter) => {
    updateNewsletter({
      id: newsletter.id,
      updates: { is_published: !newsletter.is_published, updated_at: Date.now() },
    });
  ...
  const sortedNewsletters = [...visibleNewsletters].sort(
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  const res = await apiClient.post('core/newsletters/ai-generate/', {
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {
                    newsletters.filter(
                      (n) =>
                        n.session === settings.current_session && n.term === settings.current_term
                    ).length
                  }
                </p>
