...
export const AnnouncementsView: React.FC = () => {
  ...
  const [weekAgoCutoff] = React.useState(() => Date.now() - 7 * 24 * 60 * 60 * 1000);
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {isReadOnlyRole
              ...
              : 'Manage school-wide and targeted announcements'}
          </p>
        </div>
        {!isReadOnlyRole && (
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {
                    announcements.filter((a) => {
                      return new Date(a.created_at).getTime() > weekAgoCutoff;
                    }).length
                  }
                </p>
      ...
      <div className="flex bg-gray-100 p-1 rounded-lg w-fit">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            filter === 'all' ? 'bg-white shadow text-brand-700' : 'text-gray-600'
          }`}
        >
          All
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            filter === 'pinned' ? 'bg-white shadow text-brand-700' : 'text-gray-600'
          }`}
        >
          Pinned
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            filter === 'urgent' ? 'bg-white shadow text-brand-700' : 'text-gray-600'
          }`}
        >
          Urgent
        </button>
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                      ...
                      {announcement.target === 'class'
                        ...
                        : announcement.target.charAt(0).toUpperCase() +
                          announcement.target.slice(1)}
                    </span>
                    <span>{new Date(announcement.created_at).toLocaleDateString()}</span>
                    <span className="capitalize">By {announcement.author_role}</span>
                  </div>
                </div>
                {!isReadOnlyRole && (
