...
export const BulkDiscountManager: React.FC = () => {
  ...
  const discountHistory = activityLogs.filter(
    (log: any) =>
  ...
  const { data: studentResults, isLoading: studentsLoading } = usePaginatedStudents(
    1,
    25,
    studentSearch,
    '',
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            ...
            <p className="text-brand-100 mt-1 max-w-lg">
              Apply financial scholarships, percentages, or full waivers to targeted groups or the
              entire school.
            </p>
          </div>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-white border rounded-xl p-1 mb-6 flex w-fit gap-2">
          <TabsTrigger
            value="apply"
            className="rounded-lg font-bold data-[state=active]:bg-brand-50 data-[state=active]:text-brand-600 px-6 py-2.5"
          >
                      ...
                      {groups.length > 0 ? (
                          ...
                          {groups.map((group: Types.StudentGroup) => (
                            ...
                            >
                              <div className="flex justify-between items-start mb-2">
                                <div
                                  className={`p-2 rounded-2xl ${selectedGroupIds.includes(group.id) ? 'bg-brand-600 text-white shadow-lg shadow-brand-100' : 'bg-gray-100 text-gray-500'}`}
                                >
                                  <Users className="h-4 w-4" />
                                </div>
                                {selectedGroupIds.includes(group.id) && (
                                  <CheckCircle2 className="h-5 w-5 text-brand-600 animate-in zoom-in" />
                                )}
                              </div>
                        ...
                        <div className="p-12 border-2 border-dashed border-gray-200 rounded-3xl text-center bg-gray-50/50">
                          ...
                          <p className="text-xs text-gray-500 mb-4 font-medium">
                            Create groups to reuse student lists across different waivers.
                          </p>
                          <Button
                            onClick={() => setShowCreateGroup(true)}
                            className="bg-white border text-brand-600 hover:bg-brand-50 h-9 px-6 text-xs font-black shadow-sm rounded-2xl"
                          >
                  ...
                  {scopeType === 'student' && (
                    <div className="space-y-4">
                      ...
                      <div className="max-h-64 overflow-y-auto rounded-2xl border border-gray-100 divide-y divide-gray-50 shadow-inner">
                        {studentsLoading ? (
                          <div className="p-12 text-center text-gray-400 font-black text-xs uppercase tracking-widest animate-pulse">
                            Syncing Database...
                          </div>
                        ) : students.length > 0 ? (
                          students.map((student: Types.Student) => (
                            ...
                            >
                              <div className="flex items-center gap-4">
                                <div
                                  className={`h-6 w-6 rounded-lg border-2 flex items-center justify-center transition-all ${selectedStudentIds.includes(student.id) ? 'bg-brand-600 border-brand-600' : 'border-gray-200'}`}
                                >
                  ...
                  {scopeType === 'all' && (
                    <div className="p-12 border-2 border-dashed border-brand-200 rounded-3xl text-center bg-brand-50/30">
                      ...
                      <p className="text-sm text-gray-600 font-bold max-w-sm mx-auto mt-2 leading-relaxed">
                        Warning: This policy will apply to 100% of currently active student records.
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {activeFees.map((fee) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`h-10 w-10 rounded-2xl flex items-center justify-center shadow-inner ${selectedFeeId === fee.id ? 'bg-brand-600 text-white' : 'bg-gray-50 text-gray-400'}`}
                          >
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-black text-gray-500 uppercase tracking-widest">
                        ...
                        {discountType === 'percent'
                          ? '(%)'
                          : `(${INITIAL_SETTINGS.currency_symbol})`}
                      </label>
                ...
                <label className="flex items-start gap-4 p-5 bg-amber-50 rounded-3xl border border-amber-100 cursor-pointer hover:bg-amber-100/50 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-amber-800 mt-1 font-bold leading-relaxed">
                      Applying this will overwrite any unique discounts already configured for
                      students in this scope for the selected fee item. Use for standardizing
                      records.
                    </p>
                  </div>
                </label>
        ...
        <TabsContent value="history">
          <Card className="p-8 rounded-3xl">
            <div className="flex items-center justify-between mb-8 pb-4 border-b">
              ...
              <Badge className="bg-brand-50 text-brand-700 font-black px-4 py-1.5 rounded-full border-brand-100">
                Audit Mode
              </Badge>
            </div>
            ...
            {logsLoading ? (
              ...
              <div className="space-y-4">
                {discountHistory.map((log: any) => (
                  <div
                    key={log.id}
                    className="p-6 rounded-3xl border border-gray-50 hover:border-brand-200 hover:bg-brand-50/20 transition-all flex items-start gap-5 shadow-sm"
                  >
              ...
              <div className="text-center py-32 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-sm text-gray-500 max-w-sm mx-auto mt-2 font-medium">
                  When you apply bulk waivers, the audit trail will be maintained here
                  automatically.
                </p>
              </div>
            )}
        ...
        <DialogContent className="max-w-3xl rounded-[40px] p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="p-10 pb-6 bg-white">
            <Tabs defaultValue="overview" className="w-full">
              ...
              <DialogDescription className="font-bold text-gray-400 text-base leading-relaxed">
                Please verify the financial impact on student ledgers before finalizing.
              </DialogDescription>
              ...
              <TabsContent value="overview" className="mt-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-8 bg-brand-600 rounded-[32px] text-white shadow-xl shadow-brand-200">
                    ...
                    <p className="text-xs font-bold mt-4 opacity-80 uppercase tracking-widest">
                      Students Affected
                    </p>
                  </div>
          ...
          <DialogFooter className="p-10 bg-gray-50 flex gap-4">
            <Button
              ...
              className="flex-1 rounded-3xl h-14 font-black text-gray-400 uppercase tracking-widest hover:text-gray-900"
            >
              Abort Changes
            </Button>
            <Button
        ...
        <DialogContent className="sm:max-w-md rounded-[40px] border-none p-0 overflow-hidden shadow-2xl">
          <div className="bg-brand-600 p-10 text-white relative overflow-hidden">
            <DialogHeader>
              ...
              <DialogDescription className="text-brand-100 font-bold opacity-80 mt-1">
                Define a custom student group for targeted financial aids.
              </DialogDescription>
            </DialogHeader>
