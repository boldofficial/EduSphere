...
export const DebtorReport: React.FC<DebtorReportProps> = ({
  ...
  if (selectedClass !== 'all') {
    filteredDebtors = filteredDebtors.filter(
      (d) => String(d.student.class_id) === String(selectedClass)
    );
  }
        ...
        <Card className="p-4 bg-blue-50 border-blue-100">
          ...
          <p className="text-2xl font-bold text-blue-700">
            {filteredDebtors.length > 0
              ? Utils.formatCurrency(totalOutstanding / filteredDebtors.length)
              : '₦0'}
          </p>
        </Card>
      </div>
              ...
              {filteredDebtors.map((d, index) => (
                <tr key={d.student.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right font-mono">
                    {Utils.formatCurrency(d.totalBill)}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-green-600">
                    {Utils.formatCurrency(d.totalPaid)}
                  </td>
                  <td className="px-4 py-3 text-right font-mono font-bold text-red-600">
                    {Utils.formatCurrency(d.balance)}
                  </td>
                </tr>
              ))}
              {filteredDebtors.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-gray-500">
                    {allDebtors.length === 0
                      ? '🎉 No outstanding fees - all students are up to date!'
                      : 'No debtors match your filter criteria.'}
                  </td>
                </tr>
              )}
            </tbody>
                ...
                <tr>
                  ...
                  <td className="px-4 py-3 text-right font-mono">
                    {Utils.formatCurrency(
                      filteredDebtors.reduce((s, d) => s + (Number(d.totalBill) || 0), 0)
                    )}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-green-600">
                    {Utils.formatCurrency(
                      filteredDebtors.reduce((s, d) => s + (Number(d.totalPaid) || 0), 0)
                    )}
                  </td>
