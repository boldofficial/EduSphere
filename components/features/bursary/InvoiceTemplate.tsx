...
export const InvoiceTemplate: React.FC<InvoiceTemplateProps> = ({
  ...
  const { totalBill, totalPaid } = Utils.getStudentBalance(
    student,
    fees,
    payments,
  ...
  const [invoiceTimestamp] = useState(() => Date.now());
  ...
  const invoiceNo = `INV-${settings.current_session.replace('/', '')}-${student.student_no.slice(-4)}-${invoiceTimestamp.toString().slice(-6)}`;
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
            ...
            <div>
              ...
              <p style={{ color: '#4b5563', fontSize: '10pt', margin: '0', maxWidth: '300px' }}>
                {settings.school_address}
              </p>
              <p
                style={{ color: '#4b5563', fontSize: '10pt', margin: '2px 0 0 0', fontWeight: 500 }}
              >
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ fontSize: '11pt', color: '#6b7280', margin: '8px 0 0 0', fontWeight: 600 }}>
              #{invoiceNo}
            </p>
          </div>
        </div>
        ...
        >
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '10pt', color: '#374151', margin: '4px 0' }}>
                  ID: <strong>{student.student_no}</strong> | Class:{' '}
                  <strong>{cls?.name || 'N/A'}</strong>
                </p>
                <p style={{ fontSize: '9pt', color: '#6b7280', margin: '0' }}>
                  Guardian: {student.parent_name} ({student.parent_phone})
                </p>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <table style={{ borderCollapse: 'collapse', fontSize: '10pt' }}>
              <tbody>
                <tr>
                  <td style={{ padding: '4px 12px 4px 0', color: '#6b7280', textAlign: 'right' }}>
                    Invoice Date:
                  </td>
                  <td style={{ padding: '4px 0', fontWeight: 700, color: '#111827' }}>
                ...
                <tr>
                  ...
                  <td style={{ padding: '4px 0', fontWeight: 700, color: '#DC2626' }}>
                    {formattedDueDate}
                  </td>
                </tr>
                <tr>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            {settings.invoice_notes && (
              ...
              >
                ...
                <p style={{ fontSize: '10pt', color: '#4b5563', margin: 0, lineHeight: '1.4' }}>
                  {settings.invoice_notes}
                </p>
              </div>
            )}
          ...
          <div style={{ minWidth: '280px' }}>
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px' }}>
              <tbody>
                <tr>
                  <td style={{ color: '#6b7280', fontSize: '10pt', paddingRight: '20px' }}>
                    Subtotal
                  </td>
                  <td
                    style={{
                  ...
                  >
                    {Utils.formatCurrency(totalBill)}
                  </td>
                </tr>
                {discount > 0 && (
                  <tr>
                    <td style={{ color: '#059669', fontSize: '10pt' }}>
                      Discount{' '}
                      {discountReason && (
                        <span style={{ fontSize: '8pt', opacity: 0.8 }}>({discountReason})</span>
                      )}
                    </td>
                    <td
                      style={{
                    ...
                    >
                      -{Utils.formatCurrency(discount)}
                    </td>
                  </tr>
                )}
                {studentPayments.length > 0 && (
                  <tr>
                    <td style={{ color: '#059669', fontSize: '11pt', fontWeight: 500 }}>
                      Total Paid
                    </td>
                    <td
                      style={{
              ...
              <tbody>
                {studentPayments.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td
                      style={{ padding: '8px 12px', color: '#374151', textTransform: 'capitalize' }}
                    >
                      {p.method}
                    </td>
                    <td
        ...
        {settings.show_bank_details && balance > 0 && (
          ...
          >
            ...
            >
              Please use <strong>{student.student_no}</strong> or{' '}
              <strong>{student.names.split(' ')[0]}</strong> as the payment reference.
            </div>
          </div>
        )}
        ...
        >
          <div style={{ textAlign: 'center', minWidth: '180px' }}>
            ...
            ></div>
            <p style={{ fontSize: '10pt', fontWeight: 700, color: '#1a1a1a', margin: 0 }}>
              School Bursar
            </p>
          ...
          <div style={{ textAlign: 'center', minWidth: '180px' }}>
            ...
            ></div>
            <p style={{ fontSize: '10pt', fontWeight: 700, color: '#1a1a1a', margin: 0 }}>
              {settings.head_of_school_name || 'Head of School'}
            </p>
        ...
        >
          ...
          <div style={{ fontSize: '8pt', color: '#9ca3af' }}>
            This is an official document generated by {settings.school_name} on{' '}
