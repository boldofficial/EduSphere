...
import { Printer, Check, Plus, Trash2, Link } from 'lucide-react';
...
interface BursaryModalsProps {
  ...
  onAddPayment: (p: Types.Payment, options?: MutationOptions) => void;
  onAddExpense: (e: Types.Expense, options?: MutationOptions) => void;
  onAddFee: (f: Types.FeeStructure, options?: MutationOptions) => void;
...
export const BursaryModals: React.FC<BursaryModalsProps> = ({
    ...
    onAddPayment(
      {
        ...
        amount: totalAmount,
        method: payMethod as Types.Payment['method'],
    ...
    onAddExpense(
      {
        ...
        category: expCat as Types.Expense['category'],
        date: expDate,
  ...
  const receiptStudent = receiptPayment
    ? students.find((s) => s.id === receiptPayment.student_id)
    : null;
      ...
      >
        <form onSubmit={handleExpenseSubmit} className="space-y-4">
          <Input
            ...
            placeholder="e.g. Classroom repairs"
          />
          <Input
            label="Amount"
            type="number"
      ...
      <Modal isOpen={showFeeModal} onClose={() => setShowFeeModal(false)} title="Add Fee Structure">
        <form onSubmit={handleFeeSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="feeOptional" className="text-sm text-gray-700">
              Optional Fee (e.g. Bus, Excursion)
            </label>
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded border-gray-300 text-brand-600 focus:ring-brand-500"
            />
            <label htmlFor="feeAllowPartial" className="text-sm text-gray-700">
        ...
        >
          <form onSubmit={handleDiscountSubmit} className="space-y-4">
            <Select
              label="Type"
              value={discCategory}
              onChange={(e) => setDiscCategory(e.target.value as 'discount' | 'scholarship')}
            >
            ...
            <Input
              ...
              placeholder="e.g. Staff Child, Merit Award"
            />
