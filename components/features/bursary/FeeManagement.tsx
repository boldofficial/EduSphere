        ...
        <Card className="h-full">
          ...
          <div className="space-y-1 max-h-[300px] lg:h-[550px] overflow-y-auto">
            {filteredStudents.map((s) => {
              const bal = Utils.getStudentBalance(
                s,
                fees,
                payments,
                settings.current_session,
              ...
              return (
                <div
                  ...
                  className={`p-2 lg:p-3 rounded-md cursor-pointer flex justify-between items-center text-sm transition-colors ${
                ...
                >
                  ...
                  <div
                    className={`font-mono text-xs lg:text-sm shrink-0 ml-2 ${bal > 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {applicableFees &&
                      applicableFees.map((fee) => (
                        <tr key={fee.id}>
                          ...
                          <td className="px-3 lg:px-4 py-2 text-right font-mono">
                            {Utils.formatCurrency(fee.amount)}
                          </td>
                        </tr>
                      ))}
                    <tr className="bg-gray-50 font-bold">
                      ...
                      <td className="px-3 lg:px-4 py-2 text-right">
                        {Utils.formatCurrency(totalBill + totalDiscount)}
                      </td>
                    </tr>
                    {totalDiscount > 0 && (
                      <tr className="bg-green-50 text-green-700">
                        ...
                        <td className="px-3 lg:px-4 py-2 text-right">
                          -{Utils.formatCurrency(totalDiscount)}
                        </td>
                      </tr>
                    )}
                    ...
                    <div className="space-y-2">
                      {potentialOptionalFees.map((fee) => {
                        ...
                        return (
                          <label
                            key={fee.id}
                            className="flex items-center justify-between p-2 rounded hover:bg-gray-50 border border-transparent hover:border-gray-100 cursor-pointer"
                          >
                  ...
                  {student?.discounts && student.discounts.length > 0 ? (
                    <div className="space-y-2">
                      {student.discounts
                        .filter(
                          (d) =>
                            d.session === settings.current_session &&
                            d.term === settings.current_term
                        )
                        .map((d) => (
                          <div
                            key={d.id}
                          ...
                          >
                            <div>
                              <div className="text-sm font-medium text-green-900">
                                {d.reason}{' '}
                                <span className="text-xs opacity-75">({d.category})</span>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
            ...
            >
              <div className="space-y-3 mt-2">
                ...
                {studentPayments.map((p) => {
                  ...
                  const lineItemsDesc =
                    p.lineItems?.map((item) => item.purpose).join(', ') || 'Payment';
                  return (
                    <div
                      key={p.id}
                    ...
                    >
                      <div>
                        ...
                        {p.remark && (
                          <div className="text-xs text-gray-500 italic">&quot;{p.remark}&quot;</div>
                        )}
                      </div>
                      <div className="flex gap-2">
                        <button
