...
import { CreditCard, FileText, Printer, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const StudentInvoiceView: React.FC<StudentInvoiceViewProps> = ({
  ...
  const availableTerms = settings.terms?.length
    ? settings.terms
  ...
  const studentPayments = payments
    .filter(
      (p) => p.student_id === student.id && p.session === targetSession && p.term === targetTerm
    )
    ...
    <div className="space-y-6 lg:space-y-8 bg-white min-h-screen rounded-2xl p-6 lg:p-8 border border-gray-100 shadow-sm">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-2">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-lg bg-indigo-50 text-indigo-700 text-[10px] font-bold uppercase tracking-wider border border-indigo-100">
                {targetTerm}
              </span>
        ...
        <div className="flex gap-2">
          <Button
            onClick={handlePrint}
            variant="outline"
            className="flex gap-2 border-indigo-200 text-indigo-700 hover:bg-indigo-50 rounded-xl px-4 py-2 font-semibold"
          >
            <Printer size={18} />
            Print Statement
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="relative overflow-hidden border-none shadow-lg bg-gradient-to-br from-indigo-500 to-blue-700 text-white p-6 rounded-2xl group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[10px] font-bold text-indigo-50/80 uppercase tracking-widest mb-1">
              Total Bill
            </p>
        ...
        <Card className="relative overflow-hidden border-none shadow-lg bg-gradient-to-br from-emerald-500 to-teal-700 text-white p-6 rounded-2xl group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-[10px] font-bold text-emerald-50/80 uppercase tracking-widest mb-1">
              Total Paid
            </p>
        ...
        <Card
          className={`relative overflow-hidden border-none shadow-lg p-6 rounded-2xl group text-white transition-all duration-500 ${balance > 0 ? 'bg-gradient-to-br from-rose-500 to-pink-700' : 'bg-gradient-to-br from-emerald-600 to-teal-800'}`}
        >
          <div className="absolute top-[-20%] right-[-10%] h-32 w-32 bg-white/10 rounded-full blur-3xl group-hover:scale-125 transition-transform duration-700" />
          <div className="relative z-10">
            <div className="h-12 w-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center mb-4 border border-white/30">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-indigo-500 mt-0.5 tracking-tight">
                {student.student_no}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="text-right md:text-left">
              ...
              <span
                className={`inline-flex items-center gap-1.5 font-bold uppercase tracking-tighter text-xs ${paymentStatus === 'paid' ? 'text-emerald-600' : paymentStatus === 'partial' ? 'text-amber-600' : 'text-rose-600'}`}
              >
                {paymentStatus}
              </span>
            </div>
          ...
          <div className="mb-8 p-4 bg-gray-50 rounded-xl border border-gray-100">
            <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">
              ...
              <span>
                {totalBill > 0
                  ? Math.min(
                      100,
                      Math.round(((Number(totalPaid) || 0) / (Number(totalBill) || 0)) * 100)
                    )
                  : 0}
                %
              </span>
            ...
            <div className="h-2.5 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${totalBill > 0 ? Math.min(100, ((Number(totalPaid) || 0) / (Number(totalBill) || 0)) * 100) : 0}%`,
                }}
              />
            </div>
      ...
      {settings.show_bank_details && balance > 0 && (
        <Card className="p-6 bg-slate-50 border border-slate-100 rounded-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-0.5">
                Bank Name
              </p>
            ...
            <div>
              ...
              <p className="text-xl font-bold text-indigo-600 tracking-wider">
