...
export const FinancialDashboard: React.FC<FinancialDashboardProps> = ({
  ...
  const { data: stats } = useFinancialStats(settings.current_session, settings.current_term);
  ...
  const totalIncome =
    stats?.total_collected ?? currentPayments.reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
  const totalExpenses =
    stats?.total_expenses ?? currentExpenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="relative overflow-hidden border-none shadow-md bg-gradient-to-br from-brand-600 to-indigo-800 text-white p-6">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div>
              <p className="text-brand-50/80 text-xs font-bold uppercase tracking-wider">
                Net Balance
              </p>
              <p className="text-3xl font-black mt-1 leading-none">
        ...
        <Card className="relative overflow-hidden border-none shadow-md bg-gradient-to-br from-amber-400 to-orange-600 text-white p-6">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div>
              <p className="text-amber-50/80 text-xs font-bold uppercase tracking-wider">
                Outstanding Fees
              </p>
              <p className="text-3xl font-black mt-1 leading-none">
          ...
          <div className="mt-6 w-full space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-gray-900">
                {Utils.formatCurrency(expectedIncome)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-brand-600 hover:text-brand-700 font-bold uppercase text-xs"
            >
              View All Report
            </Button>
          </div>
            ...
            {debtors.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  There are currently no outstanding fees.
                </p>
              </div>
            )}
          </div>
