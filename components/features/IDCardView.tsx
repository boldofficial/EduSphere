...
import { IDCardViewProps, CardTemplate, CardSide, ACCENT_COLORS } from './id-cards/id-card-types';
import {
  PremiumFront,
  ModernFront,
  ElegantFront,
...
export const IDCardView: React.FC<IDCardViewProps> = ({ students, classes, settings }) => {
  ...
  const renderFrontCard = (student: Types.Student) => {
    ...
    switch (template) {
      case 'premium':
        return <PremiumFront {...props} />;
      case 'modern':
        return <ModernFront {...props} />;
      case 'elegant':
        return <ElegantFront {...props} />;
      case 'classic':
        return <ClassicFront {...props} />;
      default:
        return <MinimalFront {...props} />;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-4 no-print">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            variant="secondary"
            onClick={() =>
              setShowSide(showSide === 'front' ? 'back' : showSide === 'back' ? 'both' : 'front')
            }
          >
      ...
      >
        {studentsToPrint.map((s) => (
          <div
            key={s.id}
            className="flex flex-col gap-4 items-center print:items-center print:justify-center print:min-h-0"
          >
