          ...
          <Card title="Select Student">
            <div className="space-y-4">
              ...
              <div className="space-y-1 max-h-[500px] overflow-y-auto border rounded-md">
                {activeStudents
                  .filter((s) => s.names.toLowerCase().includes(searchQuery.toLowerCase()))
                  .map((s) => (
                    <button
                      key={s.id}
                ...
                <div className="relative z-10 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-brand-100 text-sm">
                      Conduct Summary for {Utils.INITIAL_SETTINGS.current_session}
                    </p>
                  </div>
                </div>
              </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card className="p-4 flex items-center gap-4">
                  <div className="h-10 w-10 bg-amber-50 text-amber-600 rounded-lg flex items-center justify-center font-bold">
                    {(
                      studentLogs.reduce((acc: number, l: any) => acc + l.score, 0) /
                  ...
                  {studentLogs.length > 0 ? (
                    studentLogs
                      ...
                      .map((log: any) => (
                        <div
                          key={log.id}
                          className="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-brand-200 transition-colors"
                        >
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <span className="text-xs font-bold text-brand-600 uppercase tracking-wide mr-2">
                                {log.trait}
                              </span>
                            ...
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map((n) => (
                                <Star
                                  key={n}
                                  className={`h-3 w-3 ${n <= log.score ? 'text-amber-400 fill-amber-400' : 'text-gray-200'}`}
                                />
                              ))}
                            </div>
                          </div>
