...
export const GradingView: React.FC<GradingViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'broadsheet' | 'report' | 'skills' | 'publish' | 'schemes' | 'assignments'
  >('broadsheet');
  ...
  const publishedCount = useMemo(
    () => classScoresForTerm.filter((s) => s.is_passed).length,
    [classScoresForTerm]
  );
  ...
  const handleScoreChange = (studentId: string, field: 'ca1' | 'ca2' | 'exam', value: number) => {
    ...
    const row = newScore.rows[rowIndex] as Types.ScoreRow;
    row[field] = value;
    ...
    const totalScore = newScore.rows.reduce((acc: number, r: Types.ScoreRow) => acc + r.total, 0);
  ...
  const handleScoreFieldChange = <K extends keyof Types.Score>(
    studentId: string,
    field: K,
    value: Types.Score[K]
  ) => {
    ...
    if (newScore[field] !== value) {
      (newScore as Types.Score)[field] = value;
      debouncedUpsert(newScore);
    }
  };
  ...
  const handleMagicRemark = async (
    ...
    if (!score?.id) {
      addToast(
        'Please enter some scores or traits first before generating an AI remark',
        'warning'
      );
      return null;
    }
  ...
  const getRow = (studentId: string) => {
    ...
    return (
      score?.rows.find((r) => r.subject === selectedSubject) || {
        ca1: 0,
        ca2: 0,
        exam: 0,
        total: 0,
  ...
  }, [reportStudentId, scores, createEmptyScore, settings.current_session, settings.current_term]);
        ...
        <div className="relative flex flex-col gap-5 px-5 py-5 lg:flex-row lg:items-center lg:justify-between lg:px-7">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {currentClass?.name || 'No class selected'} | {settings.current_term} |{' '}
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`rounded-xl p-2 ${
                    isActive
                      ...
                      : 'bg-gray-100 text-gray-500 group-hover:bg-brand-100 group-hover:text-brand-700'
                  }`}
                >
                  {tab.key === 'publish' ? (
                ...
                <div>
                  <p
                    className={`text-sm font-semibold ${isActive ? 'text-brand-900' : 'text-gray-800'}`}
                  >
                    {tab.label}
                  </p>
