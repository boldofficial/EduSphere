...
interface TeachersViewProps {
  ...
  onUpdate: (
    params: { id: string; updates: Partial<Types.Teacher> },
    options?: { onSuccess?: () => void; onError?: (err: any) => void }
  ) => void;
...
export const TeachersView: React.FC<TeachersViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const onSuccess = () => {
      addToast(
        editingId ? 'Teacher updated successfully' : 'Teacher added successfully',
        'success'
      );
    ...
    const onError = (err: any) => {
      addToast(
        err?.response?.data?.error || err?.message || 'Transaction failed. Please check your data.',
        'error'
      );
      setIsSubmitting(false);
    };
    ...
    if (editingId) {
      onUpdate({ id: String(editingId), updates: formData }, { onSuccess, onError });
    } else {
      onAdd(formData as Types.Teacher, { onSuccess, onError });
    }
  };
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="h-14 w-14 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-lg overflow-hidden border-2 border-purple-200">
                  {t.passport_url ? (
                    <img
                      src={Utils.getMediaUrl(t.passport_url) || ''}
                      alt={t.name}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border-t pt-4 mt-4">
            <h4 className="text-sm font-bold text-brand-700 mb-3 uppercase tracking-wider">
