...
export const BroadsheetView: React.FC<BroadsheetViewProps> = ({
  ...
  const sheetData = activeStudents
    .map((s) => {
      ...
      const studentSubjects =
        s.assigned_subjects && s.assigned_subjects.length > 0 ? s.assigned_subjects : subjects;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await apiClient.get(
        `academic/classes/${selectedClass}/export-broadsheet-pdf/`,
        {
          params: {
      ...
      link.setAttribute(
        'download',
        `Broadsheet_${currentClass?.name}_${settings.current_session}_${settings.current_term}.pdf`
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center no-print">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Master Broadsheet</h1>
          <p className="text-gray-500">Class-wide results analysis</p>
        </div>
      ...
      <div className="overflow-x-auto bg-white border rounded-lg shadow-sm print:shadow-none print:border-none">
        <div className="p-4 border-b bg-gray-50 text-center print:block hidden">
          ...
          <p>
            Broadsheet Report • {currentClass?.name} • {settings.current_session}{' '}
          ...
          <tbody>
            {sheetData.map((s, idx) => (
              <tr key={s.id} className="hover:bg-gray-50">
                ...
                <td className="px-2 py-2 border text-center font-bold bg-gray-50">
                  {s.average.toFixed(1)}
                </td>
                <td className="px-2 py-2 border text-center font-bold bg-gray-50">
                  {Utils.ordinalSuffix(idx + 1)}
                </td>
              </tr>
            ))}
          </tbody>
