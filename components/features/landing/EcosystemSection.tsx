          ...
          <div className="space-y-4">
            {portals.map((portal) => (
              <button
                ...
                className={`w-full p-6 rounded-3xl text-left transition-all duration-300 flex items-center gap-6 border-2 ${
                ...
                <div>
                  <h3
                    className={`text-xl font-bold ${
                      activePortal.id === portal.id ? 'text-brand-900' : 'text-gray-500'
                    }`}
                  >
                    {portal.title}
                  </h3>
                  <p
                    className={`text-sm mt-1 leading-tight ${
                      activePortal.id === portal.id ? 'text-brand-700' : 'text-gray-400'
                    }`}
                  >
                    {portal.desc}
                  </p>
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-3xl font-black text-gray-900 mb-6">
                  ...
                  <span className="text-brand-600">
                    {activePortal.title.split(' ')[1] || activePortal.title}
                  </span>
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {activePortal.features.map((feature, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-3 bg-white p-4 rounded-2xl shadow-sm border border-brand-50 hover:border-brand-100 transition-colors group"
                    >
                ...
                <div className="mt-12 p-6 bg-white rounded-3xl border border-brand-100 shadow-sm">
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">
                    Designed with user experience at the core, this portal ensures that tasks which
                    used to take hours now happen in clicks. Fully optimized for mobile and desktop
                    access.
                  </p>
                </div>
              </div>
