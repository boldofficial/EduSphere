  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        custom_subject: '',
        custom_body: '',
        audience_filter: { role: '', school_id: '' },
      });
    } catch {
                ...
                <div className="grid grid-cols-2 gap-4">
                  {templates.map((t) => (
                    <button
                      key={t.id}
                      onClick={() =>
                        setFormData({ ...formData, template_id: t.id, custom_body: '' })
                      }
                    ...
                    >
                      <div
                        className={`w-10 h-10 rounded-2xl mb-4 flex items-center justify-center transition-all ${formData.template_id === t.id ? 'bg-brand-600 text-white scale-110 shadow-lg' : 'bg-white text-gray-400 group-hover:text-brand-600'}`}
                      >
                        <Layout size={20} />
                      </div>
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-2xl mb-4 flex items-center justify-center transition-all ${!formData.template_id ? 'bg-brand-600 text-white scale-110 shadow-lg' : 'bg-white text-gray-400 group-hover:text-brand-600'}`}
                    >
                      <Plus size={20} />
                    </div>
              ...
              <div className="p-6 bg-brand-50 border border-brand-100 rounded-[32px] flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-brand-900 opacity-70 leading-relaxed mt-1">
                    Filter exactly who should receive this campaign. Leaving filters empty will
                    target all active users on the platform.
                  </p>
                </div>
              </div>
                        ...
                        onClick={() =>
                          setFormData({
                            ...
                            audience_filter: {
                              ...
                              role: formData.audience_filter.role === role ? '' : role,
                            },
                          })
                        }
                ...
                <div className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        audience_filter: { ...formData.audience_filter, school_id: e.target.value },
                      })
                    }
                  >
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="p-6 bg-blue-50 border border-blue-100 rounded-3xl flex items-start gap-4">
                ...
                <p className="text-xs font-medium text-blue-900 opacity-80 leading-relaxed">
                  Your campaign will be created as a <strong>Draft</strong>. You can review it one
                  last time in the list before hitting Send.
                </p>
              </div>
            </div>
          )}
