...
export const NextLessonWidget: React.FC<{ teacherId: string }> = ({ teacherId }) => {
  ...
  const targetClass = useMemo(() => {
    ...
    return classes.find(
      (c) => c.id === nextLesson.student_class || c.id === nextLesson.student_class?.id
    );
  }, [nextLesson, classes]);
    ...
    return (
      <Card className="bg-brand-900 border-none overflow-hidden relative group">
        ...
        <div className="p-8 relative z-10">
          ...
          <p className="text-brand-100/70 text-sm font-medium max-w-[200px]">
            You&rsquo;ve completed your schedule for the day. Enjoy your break!
          </p>
        </div>
      </Card>
    );
