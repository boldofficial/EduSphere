  ...
  <div className="space-y-6">
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-sm font-medium text-gray-500">Total: {schools.length} Schools</div>
      </div>
...
interface PlatformSettingsTabProps {
  editedPlatformSettings: PlatformSettings;
  setEditedPlatformSettings: (v: PlatformSettings) => void;
  handleSavePlatformSettings: () => void;
}

export const DashboardPlatformSettingsTab: React.FC<PlatformSettingsTabProps> = ({
  editedPlatformSettings,
  setEditedPlatformSettings,
        ...
        <div className="p-6 bg-gradient-to-br from-purple-900 to-indigo-900 text-white rounded-2xl space-y-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                <option value="gemini" className="text-gray-900">
                  Google Gemini
                </option>
                <option value="openrouter" className="text-gray-900">
            ...
            {editedPlatformSettings?.ai_provider === 'openrouter' && (
              <>
                ...
                <div>
                  ...
                  <select
                    value={
                      editedPlatformSettings?.openrouter_model || 'google/gemini-2.0-flash-001'
                    }
                    onChange={(e) =>
                      setEditedPlatformSettings({
                        ...editedPlatformSettings,
                    ...
                    className="w-full bg-white/10 border-0 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-white text-white"
                  >
                    <option value="google/gemini-2.0-flash-001" className="text-gray-900">
                      Gemini 2.0 Flash (Free)
                    </option>
                    <option value="google/gemini-2.5-pro-preview" className="text-gray-900">
                      Gemini 2.5 Pro (Paid)
                    </option>
                    <option value="anthropic/claude-3.5-sonnet" className="text-gray-900">
                      Claude 3.5 Sonnet (Paid)
                    </option>
                    <option value="openai/gpt-4o" className="text-gray-900">
                      GPT-4o (Paid)
                    </option>
                    <option value="openai/gpt-4o-mini" className="text-gray-900">
                      GPT-4o Mini (Budget)
                    </option>
                    <option value="meta-llama/llama-3.1-70b-instruct" className="text-gray-900">
                      Llama 3.1 70B (Free)
                    </option>
                    <option value="mistralai/mistral-large" className="text-gray-900">
                      Mistral Large (Paid)
                    </option>
                    <option value="deepseek/deepseek-chat-v3-0324" className="text-gray-900">
                      DeepSeek V3 (Budget)
                    </option>
                  </select>
                </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-white/10">
              <span
                className={`h-2 w-2 rounded-full ${
                  (editedPlatformSettings?.ai_provider === 'openrouter' &&
                    editedPlatformSettings?.openrouter_api_key) ||
                  (editedPlatformSettings?.ai_provider !== 'openrouter' &&
                    editedPlatformSettings?.gemini_api_key)
...
interface SchoolModalProps {
  ...
  handleUpdateSchool: (schoolId: number, data: Partial<SchoolSummary>) => void;
}
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden animate-in zoom-in-95 duration-200">
        <div className="p-8">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-sm font-mono text-brand-600">
                .{selectedSchool.domain} (Subdomain)
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="font-bold uppercase tracking-tight">
                  {selectedSchool.subscription?.payment_method || 'N/A'}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Current Status</p>
                <p className="font-bold uppercase text-amber-600">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleUpdateSchool(selectedSchool.id, { custom_domain: customDomain })
                      }
                    >
                      Update
                    </Button>
          ...
          <div className="flex gap-3">
            ...
            <Button variant="outline" className="flex-1 h-12 text-gray-500" onClick={onClose}>
              Close
            </Button>
          </div>
        </div>
