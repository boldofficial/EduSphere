      ...
      <div className="bg-brand-900 text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-white/10 rounded-xl">
              <span className="text-brand-100 text-sm flex items-center gap-1">
        ...
        <div className="space-y-4">
          {upcomingEvents.length > 0 ? (
            upcomingEvents.map((event, i) => (
              <div
                key={event.id}
                className={`p-4 rounded-2xl border ${event.event_type === 'exam' ? 'bg-red-50 border-red-100' : event.event_type === 'holiday' ? 'bg-green-50 border-green-100' : 'bg-brand-50 border-brand-100'}`}
              >
                <p
                  className={`text-xs font-bold uppercase mb-1 ${event.event_type === 'exam' ? 'text-red-600' : event.event_type === 'holiday' ? 'text-green-600' : 'text-brand-600'}`}
                >
            ...
            <div className="p-4 bg-brand-50 rounded-2xl border border-brand-100">
              ...
              <p className="text-xs text-gray-500 mt-2">
                Make sure to complete fee payments before resumption.
              </p>
            </div>
          )}
        </div>
