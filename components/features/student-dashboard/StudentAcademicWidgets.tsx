  ...
  return (
    <div className="space-y-6">
      ...
      {nextExam && daysUntilExam !== null && (
        <div
          className={`p-3 sm:p-4 rounded-xl lg:rounded-2xl border flex items-center gap-3 sm:gap-4 ${
            daysUntilExam <= 7
      ...
      {isResultPublished && <AcademicProgressChart scores={scores} studentId={studentId} />}
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        ...
        {isResultPublished && myScore && myScore.rows.length > 0 ? (
          <div className="overflow-x-auto">
            ...
            {myScore.rows.length > 6 && (
              <p className="text-xs text-gray-400 mt-2 text-center">
                +{myScore.rows.length - 6} more subjects
              </p>
            )}
          </div>
        ) : (
          <div className="text-center py-8 text-gray-400 italic text-sm">
            {!isResultPublished
              ? 'Results pending publication by school administration.'
              : 'No scores available for this term yet.'}
          </div>
        )}
      </div>
