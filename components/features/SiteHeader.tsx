      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
            ...
            {settings.logo_media ? (
              <>
                <img
                  ...
                  alt="Logo"
                  className="h-12 w-12 rounded-xl object-contain bg-white/10"
                />
                <div
                  className={`transition-colors duration-300 ${isScrolled ? 'text-brand-900' : 'text-white'}`}
                >
                  <h1 className="text-xl font-bold leading-none">
                    {settings.school_name || 'Registra'}
                  </h1>
                  <p
                    className={`text-xs font-medium ${isScrolled ? 'text-brand-500' : 'text-white/80'}`}
                  >
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link
              href="/dashboard"
              className="px-6 py-2.5 bg-accent-500 hover:bg-accent-600 text-brand-900 font-bold rounded-xl transition-all shadow-lg hover:shadow-accent-500/30 transform hover:-translate-y-0.5"
            >
              Portal Access
            </Link>
          </div>
