...
const getFeatureIcon = (name: string) => {
  ...
  if (lowerName.includes('teacher') || lowerName.includes('expert') || lowerName.includes('staff'))
    return Users;
...
export const CoreValuesSection: React.FC<CoreValuesSectionProps> = ({ settings }) => {
  const coreValues =
    settings.landing_core_values.length > 0
      ? settings.landing_core_values
      : [
          {
            title: 'CARE',
            description:
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-gray-600">
            At {settings.school_name || 'Registra'}, these values guide everything we do — syncing
            success and inspiring excellence.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {coreValues.map((value, i) => {
            ...
            return (
              <div
                key={i}
                className={`group bg-gradient-to-br ${colorClass} rounded-3xl p-8 text-white shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 relative overflow-hidden`}
              >
