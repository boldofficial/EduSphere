...
export const PayrollWorkflow: React.FC<PayrollWorkflowProps> = ({ onViewPayslip }) => {
  ...
  const handleGenerate = () => {
    generatePayroll(selectedMonth, {
      ...
      onError: (err: any) =>
        addToast(err.response?.data?.error || 'Failed to generate payroll', 'error'),
    });
  };
  ...
  const statusConfig: Record<
    string,
    { color: string; badge: 'default' | 'secondary' | 'outline' | 'destructive' }
  > = {
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Generate your first payroll using the form above.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {payrolls.map((payroll) => {
            ...
            return (
              <Card
                key={payroll.id}
                className={`border-0 shadow-sm border-l-4 ${config.color} overflow-hidden`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div>
                        ...
                        <CardDescription>
                          {payroll.total_staff} staff • Total:{' '}
                          {formatNaira(payroll.total_wage_bill)}
                          {payroll.generated_by_name &&
                            ` • Generated by ${payroll.generated_by_name}`}
                        </CardDescription>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant={config.badge}>{payroll.status.toUpperCase()}</Badge>
