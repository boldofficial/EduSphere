...
export const HRDashboard: React.FC<HRDashboardProps> = ({ onNavigate }) => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: dashboard?.last_paid_payroll ? formatNaira(dashboard.last_paid_payroll.total) : 'None',
      subtitle: dashboard?.last_paid_payroll
        ? new Date(dashboard.last_paid_payroll.month).toLocaleDateString('en-US', {
            month: 'long',
            year: 'numeric',
          })
