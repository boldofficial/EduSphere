...
interface StaffViewProps {
  ...
  onUpdate: (
    params: { id: string; updates: Partial<Types.Staff> },
    options?: { onSuccess?: () => void; onError?: (error: Error) => void }
  ) => void;
...
export const StaffView: React.FC<StaffViewProps> = ({ staff, onAdd, onUpdate, onDelete }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    passport_url: null as string | null,
    basic_salary: 0,
    bank_name: '',
    account_number: '',
    account_name: '',
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Non-Academic Staff</h1>
          <p className="text-gray-500">Manage support staff and roles</p>
        </div>
          ...
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-2">
              {availableModules.map((module) => (
                <label
                  key={module.id}
                  className="flex items-center gap-2 p-2 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-brand-300"
                >
                  <input
                    type="checkbox"
