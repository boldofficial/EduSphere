...
export const TeacherDashboardView = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'subjects' | 'students' | 'grading' | 'attendance' | 'settings'
  >('overview');
  ...
  // Derived Data
  const myTeacherProfile = teachers.find((t) => t.email === currentUser?.email) ||
    teachers[0] || { id: 'teacher_1', name: 'Demo Teacher' };

  const myAssignments = useMemo(
    () =>
      subjectTeachers.filter(
        (st) =>
          String(st.teacher_id) === String(myTeacherProfile.id) && st.session === targetSession
      ),
  ...
  const myClassIds = useMemo(() => {
    ...
    const idsFromClassTeacher = classes
      .filter((c) => String(c.class_teacher_id) === String(myTeacherProfile.id))
      .map((c) => c.id);
  ...
  const stats = [
    ...
    },
    {
      ...
      value: classesAsClassTeacher.length.toString(),
      icon: ClipboardList,
      color: 'bg-amber-500',
    },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Welcome, <span className="text-brand-600">{myTeacherProfile.name}</span>
          </p>
                ...
                <div className="space-y-3">
                  {myClasses.map((cls) => {
                    const isClassTeacher =
                      String(cls.class_teacher_id) === String(myTeacherProfile.id);
                    const studentsInClass = students.filter(
                      (s: Types.Student) => String(s.class_id) === String(cls.id)
                    ).length;
                    const subjectsInClass = myAssignments.filter(
                      (a) => String(a.class_id) === String(cls.id)
                    );
                    return (
                      <div
                        key={cls.id}
      ...
      {activeTab === 'settings' && (
        <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 leading-relaxed">
                      This signature will be automatically attached to student report cards for your
                      classes. Please upload a clear image (PNG preferred) with a transparent or
                      white background.
                    </p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900">
                      {myTeacherProfile.staff_type === 'ACADEMIC' ? 'Academic Staff' : 'Staff'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl flex items-start gap-4">
            ...
            <div>
              <h4 className="text-amber-900 font-bold mb-1 underline decoration-amber-300">
                Important Note
              </h4>
              <p className="text-amber-800 text-sm leading-relaxed">
                Please ensure your signature is professional and clearly legible. Once uploaded, it
                will be applied to all <b>printed</b> and <b>digital</b> report cards you release
                for this term.
              </p>
            </div>
          </div>
