...
export const CalendarView: React.FC = () => {
  ...
  const filteredEvents = useMemo(() => {
    const userClassId =
      currentRole === 'student' && currentUser?.class_id ? currentUser.class_id : null;
    return events.filter((e) => {
      ...
      if (
        currentRole === 'teacher' &&
        (e.target_audience === 'teachers' || e.target_audience === 'students')
      )
        return true;
      return false;
    });
    ...
    <div className="space-y-6">
      ...
      {viewMode === 'month' ? (
        <CalendarMonthView
          ...
          onEventClick={(event) =>
            isReadOnlyRole ? handleViewEvent(event) : handleOpenModal(event)
          }
        />
      ) : (
        <CalendarAgendaView
