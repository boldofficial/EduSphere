...
export const TimetableBuilder = () => {
  ...
  React.useEffect(() => {
    if (isStudent && currentUser) {
      ...
      const targetClassId =
        profile?.current_class || currentUser?.student_id?.current_class || currentUser?.class_id;
  ...
  const handleCellClick = (day: string, periodId: string) => {
    const existing = activeTimetable?.entries.find(
      (e) => e.day_of_week === day && e.period === periodId
    );
      ...
      return (
        <div className="p-12 text-center bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col items-center">
          ...
          <p className="text-gray-500 max-w-md">
            We couldn&rsquo;t determine your assigned class. Please contact the school
            administrator.
          </p>
        </div>
      );
        ...
        return (
          <div className="p-12 text-center bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col items-center">
            ...
            <p className="text-gray-500">
              The school schedule is currently being set up. Please check back later.
            </p>
          </div>
        );
      }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200">
        ...
        <div className="flex gap-2">
          {!isStudent && (
            <Button variant="outline" size="sm" onClick={() => setSelectedClassId('')}>
              Change Class
            </Button>
          )}
          {!isStudent && (
            <>
              ...
              <Button size="sm" onClick={handlePublish} disabled={isGenerating || !activeTimetable}>
                Publish
              </Button>
            </>
          )}
          ...
          <thead>
            <tr>
              ...
              {days.map((day) => (
                <th
                  key={day}
                  className="p-3 border-b bg-gray-50 text-center text-xs font-bold text-gray-500 uppercase w-[18%]"
                >
                  {day}
                </th>
              ))}
          ...
          <tbody>
            {sortedPeriods.map((period) => (
              <tr key={period.id} className="border-b last:border-0 hover:bg-gray-50/50">
                ...
                {days.map((day) => {
                  // Find entry
                  const entry = activeTimetable?.entries.find(
                    (e) => e.day_of_week === day && e.period === period.id
                  );
                  ...
                  if (period.category !== 'Regular') {
                    return (
                      <td
                        key={day}
                        className="bg-gray-100 p-2 text-center text-gray-400 text-xs italic"
                      >
                        {period.category}
                      </td>
                    );
                        ...
                        <div className="h-full flex flex-col justify-between p-1 bg-white border border-brand-100 rounded-lg shadow-sm group-hover:border-brand-300">
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            ...
                            <span className="text-xs text-gray-600 truncate">
                              {teachers.find((t) => String(t.id) === String(entry.teacher))?.name ||
                                entry.teacher_name ||
                                'No Teacher'}
                            </span>
                          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex justify-end gap-3 mt-8">
              ...
              <Button onClick={handleSaveEntry} disabled={!formSubjectId}>
                Save Entry
              </Button>
            </div>
          </div>
