                      ...
                      {q.question_type === 'mcq' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                          {q.options?.map((opt: any) => (
                            ...
                            >
                              <div
                                className={`w-3 h-3 rounded-full border ${opt.is_correct ? 'bg-green-500 border-green-500' : 'border-gray-300'}`}
                              />
                              {opt.text}
                              {opt.is_correct && (
                                <span className="ml-auto text-xs font-bold uppercase tracking-wider text-green-600">
                                  Correct
                                </span>
                              )}
                            </div>
