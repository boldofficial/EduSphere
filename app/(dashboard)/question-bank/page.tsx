...
export default function QuestionBankPage() {
  ...
  const { data: questions = [], isLoading: questionsLoading } = useQuery({
    ...
    queryFn: () =>
      selectedBank
        ? fetchAll<any>(`learning/question-banks/${selectedBank.id}/questions/`)
        : Promise.resolve([]),
  ...
  const addQuestion = useMutation({
    mutationFn: async (data: typeof questionForm) => {
      const response = await apiClient.post(
        `learning/question-banks/${selectedBank.id}/questions/`,
        data
      );
      return response.data;
    },
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              {view === 'banks'
                ? 'Create and manage reusable questions for exams'
      ...
      >
        <form onSubmit={handleAddQuestion} className="space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            <Button type="button" variant="secondary" onClick={() => setShowQuestionModal(false)}>
              Cancel
            </Button>
            <Button
              type="submit"
      ...
      <Modal
        ...
        title="Import Questions from Past Exams"
        size="lg"
      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setImportForm({ ...importForm, count: parseInt(e.target.value) || 10 })
                }
              />
            </div>
          </div>
          <div className="bg-blue-50 p-3 rounded-lg text-sm text-blue-700">
            💡 This connects to ALOC/QBoard API to fetch real past questions. Free tier: 7,000
            calls/month
          </div>
          {!selectedBank && (
            <p className="text-orange-500 text-sm">
              ⚠️ Please save the question bank first before importing questions.
            </p>
          )}
          <div className="flex gap-2 justify-end">
            <Button
          ...
          {importedQuestions.length > 0 && (
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 flex justify-between items-center">
                ...
                >
                  {saveImportedQuestions.isPending
                    ? 'Importing...'
                    : `Import All to ${selectedBank?.name}`}
                </Button>
              </div>
              <div className="max-h-60 overflow-y-auto">
                {importedQuestions.slice(0, 5).map((q, i) => (
                  <div key={i} className="p-3 border-b text-sm">
                    <p className="font-medium">
                      {i + 1}. {q.question_text.substring(0, 80)}...
                    </p>
                ...
                {importedQuestions.length > 5 && (
                  <p className="p-3 text-gray-500 text-center text-sm">
                    + {importedQuestions.length - 5} more questions
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-3xl font-bold">
                  {banks.reduce((acc: number, b: any) => acc + (b.questions_count || 0), 0)}
                </p>
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-4 rounded-xl text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-3xl font-bold">
                  {new Set(banks.map((b: any) => b.subject_name)).size}
                </p>
          ...
          {banksLoading ? (
            ...
            filteredBanks.map((bank: any) => (
              ...
              >
                ...
                <h3 className="font-bold text-lg text-gray-900 group-hover:text-brand-600 transition">
                  {bank.name}
                </h3>
        ...
        <div className="space-y-4">
          {questionsLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">
                Add questions manually or import from past exams
              </p>
            </div>
          ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${getDifficultyColor(q.difficulty)}`}
                      >
                        {q.difficulty}
                      </span>
                      {q.topic && (
