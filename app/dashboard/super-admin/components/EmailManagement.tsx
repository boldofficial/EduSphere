    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] overflow-hidden">
        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium">
              Template: <span className="font-bold text-brand-600">{template?.name}</span>
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-8 bg-gray-50 border-t border-gray-100 flex gap-4">
          <button
            onClick={onClose}
            className="flex-1 py-4 text-gray-400 font-black uppercase text-xs tracking-widest hover:bg-white rounded-2xl transition-all"
          >
            Cancel
          </button>
          <button
...
export function EmailTemplatesTab({
  ...
  templates: any[];
  onTemplatesChanged?: () => Promise<unknown> | void;
}) {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm">
            Customize platform-wide automated emails using dynamic variables.
          </p>
        </div>
      </div>
            ...
            <div className="relative z-10">
              ...
              <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs font-black text-gray-900 uppercase tracking-widest hover:text-brand-600 transition-colors"
                >
                  Edit Template
                </button>
              </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {logs.map((log: any) => (
              <tr key={log.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-xs font-black uppercase tracking-widest text-brand-600">
