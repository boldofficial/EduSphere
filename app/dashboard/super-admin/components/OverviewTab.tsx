  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col gap-4 hover:shadow-lg transition-all group">
      <div className="flex items-center gap-4">
        <div
          className={`w-12 h-12 ${color} rounded-xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform`}
        >
          <Icon size={22} />
        </div>
        <div>
...
export function OverviewTab({ schools, revenue, health, strategic, governance }: any) {
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <HealthCard
          ...
          icon={Activity}
        />
        <HealthCard title="Security Shield" status="healthy" message="Hardened" icon={Shield} />
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm mt-8">
        ...
        <div className="space-y-4">
          {activities.slice(0, 5).map((log: any) => (
            <div
              key={log.id}
              className="flex items-start gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:border-brand-200 transition-all"
            >
              <div className="mt-1">
