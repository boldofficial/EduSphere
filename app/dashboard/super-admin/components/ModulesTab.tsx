    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-gray-500 font-medium text-sm">
          Global master switches. Turning a module OFF here disables it platform-wide.
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {modules.map((mod) => (
          <div
            key={mod.id}
            className={`bg-white p-6 rounded-3xl border transition-all group ${mod.is_active ? 'border-gray-100 shadow-sm hover:shadow-xl hover:shadow-brand-900/5' : 'border-dashed border-gray-200 opacity-60 grayscale'}`}
          >
            <div className="flex items-start justify-between mb-6">
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${mod.is_active ? 'bg-brand-50 text-brand-600' : 'bg-gray-100 text-gray-400'}`}
              >
                <Zap size={28} />
              </div>
              <button
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform duration-200 ${mod.is_active ? 'translate-x-6' : 'translate-x-0'}`}
                ></div>
              </button>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between">
              ...
              <div className="flex -space-x-2">
                <div
                  className={`w-6 h-6 rounded-full border-2 border-white ${mod.is_active ? 'bg-brand-200' : 'bg-gray-200'}`}
                ></div>
                <div
                  className={`w-6 h-6 rounded-full border-2 border-white ${mod.is_active ? 'bg-brand-100' : 'bg-gray-100'}`}
                ></div>
              </div>
            </div>
          </div>
