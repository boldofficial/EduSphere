  ...
  const filteredSchools = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (s: any) => s.name?.toLowerCase().includes(q) || s.domain?.toLowerCase().includes(q)
      );
    }
    if (statusFilter !== 'all') {
  ...
  const executePendingAction = async () => {
    ...
    try {
      if (pendingAction.action === 'delete') {
        ...
        await apiClient.patch(`schools/manage/${pendingAction.schoolId}/`, {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <>
                      ...
                      <div className="absolute right-6 top-12 z-20 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 w-48 animate-in zoom-in-95 duration-150">
                        ...
                        {school.subscription_status === 'active' ? (
                          <button
                            onClick={() => {
                              handleAction(school.id, 'suspend');
                              setOpenDropdown(null);
                            }}
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
            <span className="text-xs font-bold text-gray-400">
              ...
              {Math.min(currentPage * PAGE_SIZE, filteredSchools.length)} of{' '}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <h3 className="text-xl font-black text-gray-900 mb-6">
              Record Payment for {selectedSchool?.name}
            </h3>
            <form onSubmit={handlePayment} className="space-y-5">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button
                  ...
                  className="px-5 py-3 text-gray-400 hover:bg-gray-50 rounded-xl font-bold text-xs uppercase tracking-widest transition-all"
                >
                  Cancel
                </button>
                <button
      ...
      <ConfirmActionModal
        ...
        message={
          pendingAction ? `Are you sure you want to ${pendingAction.action} this school?` : ''
        }
