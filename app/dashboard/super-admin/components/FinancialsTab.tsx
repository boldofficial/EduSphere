  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-lg transition-all group relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div
          className={`w-11 h-11 ${gradient} rounded-xl flex items-center justify-center text-white shadow-lg mb-4`}
        >
          <Icon size={20} />
        </div>
...
export function FinancialsTab({ revenue }: any) {
  ...
  const thisMonthRevenue =
    monthlyData.length > 0 ? monthlyData[monthlyData.length - 1]?.value || 0 : 0;
  const lastMonthRevenue =
    monthlyData.length > 1 ? monthlyData[monthlyData.length - 2]?.value || 0 : 0;
  const monthGrowth =
    lastMonthRevenue > 0
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50/80 border-b border-gray-100">
              <tr>
                <th className="px-6 py-3.5 font-bold text-gray-500 text-xs uppercase tracking-wider">
                  School
                </th>
                <th className="px-6 py-3.5 font-bold text-gray-500 text-xs uppercase tracking-wider">
                  Amount
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    ...
                    <p className="text-sm text-gray-300 mt-1">
                      Payments will appear here as schools subscribe.
                    </p>
                  </td>
                </tr>
              )}
