  ...
  useEffect(() => {
    const loadPackage = async () => {
      ...
      try {
        ...
        const packages = await DataService.fetchAll<any>('bursary/admission-packages', {
        ...
        <main className="flex-grow flex items-center justify-center py-32 px-4">
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center max-w-lg">
            ...
            <p className="text-gray-600 mb-8">
              Thank you for applying to {settings.school_name}. We have received your application
              and will contact you shortly at <strong>{formData.parent_email}</strong>.
            </p>
            <Link
              href="/"
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 max-w-xl mx-auto">
              Complete the form below to apply for admission to {settings.school_name}. We look
              forward to welcoming your child!
            </p>
          </div>
          ...
          >
                  ...
                  >
                    ...
                    {intakes.map((i) => (
                      <option key={i.id} value={i.id}>
                        {i.name} ({new Date(i.start_date).getFullYear()})
                      </option>
                    ))}
                  </select>
                  {intakes.length === 0 && (
                    <p className="text-xs text-red-500 mt-1">
                      No active admission intakes at this time. Please contact the school.
                    </p>
                  )}
                </div>
                <div>
            ...
            {packageFees.length > 0 && (
              <div>
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Select the items you wish to purchase with this admission.
                </p>
                <div className="space-y-3">
                  {packageFees.map((fee: any) => (
                    <label
                      key={fee.id}
                      className={`flex items-start p-4 border rounded-xl cursor-pointer transition-all ${selectedFees.has(fee.id) ? 'border-brand-500 bg-brand-50' : 'border-gray-200 hover:border-brand-200'}`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div className="ml-3 flex-1">
                        <div className="flex justify-between">
                          <span
                            className={`font-medium ${selectedFees.has(fee.id) ? 'text-brand-900' : 'text-gray-700'}`}
                          >
                            {fee.category_name}{' '}
                            {!fee.is_optional && (
                              <span className="text-xs text-brand-600 bg-brand-100 px-2 py-0.5 rounded ml-2">
                                Mandatory
                              </span>
                            )}
                          </span>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                          {fee.name !== fee.category_name ? fee.name : 'Standard Fee'}
                        </p>
                      </div>
                    </label>
                  ))}
            ...
            <div className="pt-4 border-t mt-8">
              ...
              <button
                ...
                className="w-full py-4 bg-accent-500 hover:bg-accent-600 text-brand-950 text-lg font-bold rounded-xl transition-all shadow-lg hover:shadow-accent-500/30 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
